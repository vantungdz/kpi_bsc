...
import { KpiInventoryDto, KpiPerformanceOverviewDto } from './dto/dashboard.dto';
...
export class DashboardsController {
  ...
  async getKpiAwaitingApprovalStats(@Req() req: Request & { user?: Employee }): Promise<any> {
  ...
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  async getAverageApprovalTimeStats(@Req() req: Request & { user?: Employee }): Promise<any> {
    const user = this.validateUser(req);
  ...
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Lấy thống kê người dùng/bộ phận có nhiều KPI đang chờ duyệt nhất' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Số lượng top muốn lấy (mặc định 5)' })
  ...
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Lấy thống kê người dùng/bộ phận submit KPI nhiều nhất/ít nhất' })
  @ApiQuery({ name: 'days', required: false, type: Number, description: 'Số ngày gần đây (mặc định 30)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Số lượng top (mặc định 5)' })
  @ApiQuery({ name: 'entityType', required: true, enum: ['user', 'section', 'department'], description: 'Loại thực thể (user, section, department)' })
  @ApiQuery({ name: 'orderBy', required: true, enum: ['most', 'least'], description: 'Sắp xếp (most, least)' })
  @ApiQuery({ name: 'recentKpisLimit', required: false, type: Number, description: 'Số KPI gần nhất cho mỗi đối tượng (mặc định 3)' })
  ...
  async getKpiSubmissionStats(
    ...
    @Query('recentKpisLimit', new DefaultValuePipe(3), ParseIntPipe) recentKpisLimit: number,
  ): Promise<any> {
    const user = this.validateUser(req);
    this.validateQueryParam(entityType, ['user', 'section', 'department'], 'entityType');
    this.validateQueryParam(orderBy, ['most', 'least'], 'orderBy');
    return this.dashboardsService.getKpiSubmissionStats(user, { days, limit, entityType, orderBy, recentKpisLimit });
  }

  @Get('statistics/kpi-performance-overview')
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Lấy tổng quan hiệu suất KPI' })
  @ApiQuery({ name: 'daysForNotUpdated', required: false, type: Number, description: 'Số ngày để coi là "chưa cập nhật gần đây" (mặc định 7)' })
  @ApiResponse({ status: 200, description: 'Dữ liệu tổng quan hiệu suất KPI.', type: KpiPerformanceOverviewDto })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  async getKpiPerformanceOverview(
    @Req() req: Request & { user?: Employee },
    @Query('daysForNotUpdated', new DefaultValuePipe(7), ParseIntPipe) daysForNotUpdated: number,
  ): Promise<KpiPerformanceOverviewDto> {
    const user = this.validateUser(req);
    return this.dashboardsService.getKpiPerformanceOverview(user, daysForNotUpdated);
  }

  @Get('statistics/kpi-inventory-stats')
  @Roles('admin', 'manager')
  @ApiOperation({ summary: 'Lấy tổng quan kho KPI (định nghĩa và lượt giao)' })
  @ApiResponse({ status: 200, description: 'Dữ liệu tổng quan kho KPI.', type: KpiInventoryDto })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  async getKpiInventoryStats(@Req() req: Request & { user?: Employee }): Promise<KpiInventoryDto> {
    const user = this.validateUser(req);
