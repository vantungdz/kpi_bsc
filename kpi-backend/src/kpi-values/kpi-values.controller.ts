...
export class KpiValuesController {
  ...
  async getMyPendingApprovals(
    ...
    const shouldGroup = groupBy === 'employee';
    const shouldIncludeReview = includeReview === 'true';

    if (shouldIncludeReview) {
      return this.kpiValuesService.getPendingApprovalsWithReview(
        req.user,
        shouldGroup,
      );
    }

    return this.kpiValuesService.getPendingApprovals(req.user, shouldGroup);
  }
  ...
  async submitProgressUpdate(
    ...
    return this.kpiValuesService.submitProgressUpdate(
      ...
      req.user.id,
      updateData.selfScore,
      updateData.selfComment,
    );
  }

  @Post('assignments/:assignmentId/draft')
  @ApiOperation({ summary: 'Save KPI value as draft' })
  @ApiResponse({ status: 201, type: KpiValue })
  @ApiResponse({ status: 404, description: 'Assignment does not exist.' })
  async saveDraftProgressUpdate(
    ...
    return this.kpiValuesService.saveDraftProgressUpdate(
      assignmentId,
      updateData.notes,
      updateData.project_details,
      req.user.id,
  ...
  @Post('batch-submit-drafts')
  @ApiOperation({ summary: 'Batch submit multiple draft KPI values' })
  @ApiResponse({ status: 200, description: 'Batch submit result' })
  async batchSubmitDrafts(
    @Body() data: { assignmentIds: number[] },
    @Req() req: Request & { user?: Employee },
  ): Promise<{ success: number[]; failed: Array<{ id: number; reason: string }> }> {
    if (!req.user?.id) {
      throw new UnauthorizedException('User not authenticated.');
    }
    return this.kpiValuesService.batchSubmitDrafts(
      data.assignmentIds,
      req.user.id,
    );
  }
  ...
  async approveByDepartment(
    ...
    const result = await this.kpiValuesService.approveValueByDepartment(
      valueId,
      req.user.id,
      body?.departmentScore,
      body?.departmentComment,
    );
    await this.auditLogService.logAction({
