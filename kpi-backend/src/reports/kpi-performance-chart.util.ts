...
export async function renderKpiPerformancePieChart(achievedCount: number, notAchievedCount: number): Promise<Buffer> {
  const width = 500; // px
  const height = 300; // px
  ...
  const configuration = {
    ...
    options: {
      plugins: {
        ...
        title: { display: true, text: 'Tỷ lệ KPI đạt mục tiêu' },
      },
    },
  };
