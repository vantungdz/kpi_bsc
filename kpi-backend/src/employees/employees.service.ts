...
import {
  EvaluationPhase,
  KpiReview,
} from 'src/evaluation/entities/kpi-review.entity';
import { effectiveManagerScoreForReporting } from 'src/evaluation/annual-review-score.util';
...
import { ReviewCycle } from 'src/review-cycle/entities/review-cycle.entity';
...
export class EmployeesService {
  ...
  async findAll(
    ...
    // Handle cycle filter - get employees created/updated within cycle date range
    let cycleStartDate: Date | undefined;
    let cycleEndDate: Date | undefined;
    let cycleIdForScore: string | undefined;

    // Check both cycle parameter and cycleId in filterOptions
    const cycleParam = cycle;
    
    if (cycleParam) {
      // Parse cycle as number
      const cycleId = parseInt(cycleParam, 10);
      
      if (!isNaN(cycleId)) {
        // Get the review cycle to retrieve startDate and endDate
        const reviewCycle = await this.reviewCycleRepository.findOne({
          where: { id: cycleId },
        });
        
        if (reviewCycle) {
          cycleStartDate = reviewCycle.startDate;
          cycleEndDate = reviewCycle.endDate;
          cycleIdForScore = cycleId.toString();
          
          this.logger.log(
            `Filtering employees by cycle ${cycleId}: ${cycleStartDate} to ${cycleEndDate}`,
          );
        } else {
          this.logger.warn(
    ...
    if (loggedInUser) {
      ...
      if (
        userHasPermission(loggedInUser, 'view', 'employee', 'company') ||
        ...
        userHasPermission(loggedInUser, 'view', 'kpi', 'employee')
      ) {
    ...
    // Apply cycle date range filter - filter employees by created_at or updated_at within cycle dates
    // We'll use a query builder for more complex date filtering
    let result: Employee[];
    
    if (cycleStartDate && cycleEndDate) {
      // Use query builder to filter by date range
      const queryBuilder = this.employeeRepository.createQueryBuilder('employee')
        .leftJoinAndSelect('employee.department', 'department')
        .leftJoinAndSelect('employee.section', 'section')
      ...
      // Apply date filter: created_at or updated_at within cycle range
      queryBuilder.andWhere(
        '(employee.created_at BETWEEN :startDate AND :endDate OR employee.updated_at BETWEEN :startDate AND :endDate)',
        {
      ...
      // Apply other filters
      if ((findOptions.where as any).departmentId) {
        queryBuilder.andWhere('employee.departmentId = :departmentId', {
          departmentId: (findOptions.where as any).departmentId,
        });
      }
      if ((findOptions.where as any).sectionId) {
        queryBuilder.andWhere('employee.sectionId = :sectionId', {
          sectionId: (findOptions.where as any).sectionId,
        });
      }
      if ((findOptions.where as any).teamId) {
        queryBuilder.andWhere('employee.teamId = :teamId', {
          teamId: (findOptions.where as any).teamId,
        });
      }
      if ((findOptions.where as any).id) {
        queryBuilder.andWhere('employee.id = :id', {
          id: (findOptions.where as any).id,
        });
      }

      result = await queryBuilder.getMany();
    } else {
      // No cycle filter, use standard find
      result = await this.employeeRepository.find(findOptions);
    }

    // Calculate totalWeightScore if cycle is provided
    if (cycleIdForScore || cycle) {
      const employeeIds = result.map(emp => emp.id);
      
      if (employeeIds.length > 0) {
        // Get reviews for calculating totalWeightScore (managerScore × weight)
        const reviews = await this.kpiReviewRepository.find({
          where: {
            employee: { id: In(employeeIds) },
            cycle: cycleIdForScore || cycle,
          },
          relations: ['kpi', 'employee'],
        });

        // Chỉ tính các (employee, kpi) có ít nhất một kpiValue APPROVED và KPI thuộc cycle (trùng khoảng ngày)
        ...
        const approvedEmployeeKpiSet = new Set<string>();
        const cycleStart = cycleStartDate ? new Date(cycleStartDate).getTime() : null;
        const cycleEnd = cycleEndDate ? new Date(cycleEndDate).getTime() : null;
        for (const v of approvedValues) {
          const a = v.kpiAssignment;
          if (!a) continue;
          const eid = a.assigned_to_employee ?? (a as any).employee_id;
          const kid = a.kpi_id;
          if (eid == null || kid == null || !employeeIdsSet.has(Number(eid))) continue;
          // Chỉ thêm nếu KPI trùng khoảng ngày cycle (giống modal getEmployeeKpis)
          ...
          approvedEmployeeKpiSet.add(`${eid}-${kid}`);
        }

        // totalWeightScore = sum(managerScore × weight) chỉ cho (employee, kpi) có kpiValue APPROVED
        const scoreMap = new Map<number, number>();
        const totalWeightMap = new Map<number, number>();
        ...
        for (const review of reviews) {
          ...
          if (employeeId == null || kpiId == null) continue;
          const key = `${employeeId}-${kpiId}`;
          const g = reviewGroups.get(key) ?? {};
        ...
        for (const [key, { mid, yearEnd }] of reviewGroups) {
          const [employeeIdStr, kpiIdStr] = key.split('-');
          const employeeId = Number(employeeIdStr);
          const kpiId = Number(kpiIdStr);
          if (!approvedEmployeeKpiSet.has(`${employeeId}-${kpiId}`)) continue;
          const kpi = mid?.kpi ?? yearEnd?.kpi;
          const kpiWeight = kpi?.weight;
          if (kpiWeight == null || kpiWeight === undefined) continue;
          ...
          const score =
            managerScore != null && managerScore !== undefined
              ? Number(managerScore) * weight
              : 0;
          const currentTotal = scoreMap.get(employeeId) || 0;
          scoreMap.set(employeeId, currentTotal + score);
          const currentWeight = totalWeightMap.get(employeeId) || 0;
          totalWeightMap.set(employeeId, currentWeight + weight);
        }

        // Add totalWeightScore and averageScore to each employee
        // averageScore = totalWeightScore / totalWeight
        return result.map(employee => {
          const totalWeightScore = scoreMap.get(employee.id) || 0;
          const totalWeight = totalWeightMap.get(employee.id) || 0;
          
          const averageScore = totalWeight > 0 
            ? parseFloat((totalWeightScore / totalWeight).toFixed(2))
            : 0;
  ...
  async findOneWithPermissions(id: number): Promise<any> {
    try {
      ...
      // Handle case where sort_order column doesn't exist in database yet
      // This can happen when synchronize: true hasn't run or database was reset
      if (error.message && error.message.includes('sort_order')) {
        console.warn(
          'sort_order column missing in departments/sections. Loading user without relations.',
        );
        // Fallback: load user without department/section relations
        const user = await this.employeeRepository.findOne({
          where: { id },
        ...
        // Try to load department and section with explicit column selection
        if (user.departmentId) {
          try {
            const deptRepo = this.employeeRepository.manager.getRepository('Department');
            const department = await deptRepo
              .createQueryBuilder('d')
              .select(['d.id', 'd.name', 'd.managerId'])
              .where('d.id = :id', { id: user.departmentId })
              .getOne();
            if (department) {
              user.department = department as any;
            }
          } catch (e) {
            console.warn('Failed to load department:', e.message);
          }
        }
        
        if (user.sectionId) {
          try {
            const sectionRepo = this.employeeRepository.manager.getRepository('Section');
            const section = await sectionRepo
              .createQueryBuilder('s')
              .select(['s.id', 's.name', 's.managerId', 's.departmentId'])
              .where('s.id = :id', { id: user.sectionId })
              .getOne();
            if (section) {
              user.section = section as any;
            }
          } catch (e) {
            console.warn('Failed to load section:', e.message);
          }
        }
  ...
  async getEmployeePerformanceHistory(
    ...
    for (const year of Object.keys(yearMap).map(Number).sort()) {
      ...
      const highlightComments = reviews
        ...
        .filter((comment): comment is string => comment !== null && comment !== undefined && comment !== '');
      years.push({
        year,
        averageKpiScore,
        achievedCount,
