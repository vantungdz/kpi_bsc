...
export class PersonalGoalService {
  ...
  async create(employeeId: number, dto: CreatePersonalGoalDto) {
    ...
    if (kpiIds.length > 0) {
      const links = kpiIds.map((kpiId) =>
        this.personalGoalKpiRepo.create({
          personalGoalId: savedGoal.id,
          kpiId,
        }),
      );
      await this.personalGoalKpiRepo.save(links);
    }
    return this.findOne(savedGoal.id, employeeId);
  }
  ...
  async update(id: number, employeeId: number, dto: UpdatePersonalGoalDto) {
    ...
    if (kpiIds.length > 0) {
      const links = kpiIds.map((kpiId) =>
        this.personalGoalKpiRepo.create({ personalGoalId: id, kpiId }),
      );
      await this.personalGoalKpiRepo.save(links);
    }
  ...
  async findAllByEmployee(employeeId: number) {
    ...
    const allKpiIds = goals.flatMap(
      (g) => g.kpiLinks?.map((l) => l.kpiId) || [],
    );
      ...
      if (goal.kpiLinks && goal.kpiLinks.length > 0 && goal.targetValue) {
        for (const link of goal.kpiLinks) {
          ...
          if (
            ...
            if (approved) {
              const percent =
                (Number(approved.value) / Number(goal.targetValue)) * 100;
    ...
    return goals.map((g) => ({
      ...g,
      kpiIds: g.kpiLinks?.map((l) => l.kpiId) || [],
    }));
  }
  ...
  async findOne(id: number, employeeId?: number) {
    const goal = await this.personalGoalRepo.findOne({
      where: { id },
      relations: ['kpiLinks'],
    });
    if (!goal) throw new NotFoundException('Personal goal not found');
    if (employeeId && goal.employeeId !== employeeId)
