...
export class KpiReviewController {
  ...
  async getMyKpisForReview(@Req() req: Request, @Query('cycle') cycle: string) {
    return this.kpiReviewService.getMyKpisForReview(
      (req as any).user.id,
      cycle,
    );
  }
  ...
  @UseGuards(JwtAuthGuard)
  @Get('annual-score')
  async getAnnualScore(
    @Query('assignmentId', ParseIntPipe) assignmentId: number,
    @Query('cycle') cycle: string,
  ) {
    return this.kpiReviewService.calculateAnnualFinalScore(assignmentId, cycle);
  }

  @UseGuards(JwtAuthGuard)
  @Get('assignment/:assignmentId')
  async getKpiReviewByAssignment(
    @Param('assignmentId', ParseIntPipe) assignmentId: number,
    @Req() req: Request,
  ) {
    return this.kpiReviewService.getKpiReviewByAssignment(assignmentId);
  }
  ...
  async submitMyKpiSelfReview(@Req() req: Request, @Body() body: any) {
    return this.kpiReviewService.submitMyKpiSelfReview(
      (req as any).user.id,
      body,
    );
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Get(':id')
  async getKpiReviewById(@Param('id', ParseIntPipe) id: number) {
  ...
  async submitSectionReview(
    ...
    return this.kpiReviewService.submitSectionReview(
      (req as any).user.id,
      body,
      (req as any).user,
    );
  ...
  async submitDepartmentReview(
    ...
    return this.kpiReviewService.submitDepartmentReview(
      (req as any).user.id,
      body,
      (req as any).user,
    );
  ...
  async submitManagerReview(
    ...
    return this.kpiReviewService.submitManagerReview(
      (req as any).user.id,
      body,
      (req as any).user,
    );
  ...
  async completeReview(
    @Req() req: Request,
    @Body() body: { reviewId: number },
  ) {
    return this.kpiReviewService.completeReview(
      body.reviewId,
      (req as any).user.id,
    );
  }
  ...
  async submitEmployeeFeedback(@Req() req: Request, @Body() body: any) {
    return this.kpiReviewService.submitEmployeeFeedback(
      (req as any).user.id,
      body,
    );
  }
  ...
  async submitReviewByRole(
    @Req() req: Request,
    @Body() body: { reviewId: number; score: number; comment: string },
  ) {
    return this.kpiReviewService.submitReviewByRole(
      (req as any).user.id,
      (req as any).user,
      body,
    );
  ...
  async rejectReviewByRole(
    @Req() req: Request,
    @Body() body: { reviewId: number; rejectionReason: string },
  ) {
    return this.kpiReviewService.rejectReviewByRole(
      (req as any).user.id,
      (req as any).user,
      body,
    );
  ...
  async batchApproveReviews(
    ...
    return this.kpiReviewService.batchApproveReviews(
      (req as any).user.id,
      body.employeeIds,
      body.level,
      (req as any).user,
