...
export class AuditLogService {
  ...
  async logAction(params: {
    ...
    if (!username && userId) {
      try {
        ...
        const employeeRepo =
          this.auditLogRepo.manager.getRepository('Employee');
  ...
  async getLogs(query: {
    ...
    if (query.username)
      qb.andWhere('LOWER(log.username) LIKE :username', {
    ...
    if (query.resource)
      qb.andWhere('LOWER(log.resource) LIKE :resource', {
        resource: `%${query.resource.toLowerCase()}%`,
      });
    if (query.fromDate)
