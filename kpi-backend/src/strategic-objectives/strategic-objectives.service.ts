...
export class StrategicObjectivesService {
  ...
  async remove(id: number): Promise<void> {
    // Get the entity directly without enrichment to avoid issues with computed properties
    const obj = await this.strategicObjectiveRepository.findOne({
      where: { id },
      relations: ['perspective', 'kpis'],
    });
    ...
    // Use the entity id instead of the whole object to avoid serialization issues
    const linkedKpis = await this.kpiRepository.find({
      where: { strategicObjective: { id: obj.id } },
    });
    if (linkedKpis.length > 0) {
      await this.kpiRepository.update(
        { strategicObjective: { id: obj.id } },
        { strategicObjective: null } as any,
      );
    }
