...
export class EmployeeSkillService {
  ...
  async create(dto: CreateEmployeeSkillDto) {
    ...
    if (!employee || !competency)
      throw new NotFoundException('Employee or Competency not found');
    const exist = await this.employeeSkillRepo.findOne({
      where: {
        employee: { id: dto.employeeId },
        competency: { id: dto.competencyId },
      },
    });
    if (exist)
      throw new NotFoundException('Skill already exists for this employee');
