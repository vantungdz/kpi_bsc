<template>
  <a-layout-header class="app-header-container">
    <div class="header-left">
      ...
      <div v-if="isUserAuthenticated" class="header-review-cycle">
        <span class="header-review-cycle-label">{{
          $t("headerReviewCycleLabel")
        }}</span>
        <a-select
          ref="headerCycleSelectRef"
          ...
          class="header-review-cycle-select"
          :placeholder="$t('selectKpiCycle')"
          @update:value="onCycleChange"
          @dropdown-visible-change="onHeaderCycleDropdownVisible"
        />
      </div>
    </div>
...
const cycleOptions = computed(
  () => store.getters["reviewCycle/cycleSelectOptions"],
);
const cyclesLoading = computed(() => store.state.reviewCycle.loading);

const headerCycleSelectRef = ref(null);

const actualUser = computed(() => store.getters["auth/user"]);

function loadReviewCycles() {
  if (store.getters["auth/isAuthenticated"]) {
    store.dispatch("reviewCycle/fetchCycles");
  }
}

function onCycleChange(value) {
  store.dispatch("reviewCycle/setSelectedCycleId", value ?? null);
}

/** After selecting or closing the dropdown, blur the Select component to remove the caret and focus ring. */
function onHeaderCycleDropdownVisible(open) {
  if (open) return;
  nextTick(() => {
    headerCycleSelectRef.value?.blur?.();
  });
}
...
watch(isUserAuthenticated, (ok) => {
  if (ok) loadReviewCycles();
});

const displayName = computed(() => {
  if (!actualUser.value) return $t("user");
  return [actualUser.value.first_name, actualUser.value.last_name].join(' ').trim() || actualUser.value.username || $t("user");
});
