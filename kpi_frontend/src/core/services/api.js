...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response) {
      ...
      const { serverMessage, message } = error.response.data;
      ...
      if (status === 401) {
        ...
        // Không tự động tạo notification cho lỗi 400
        // Component sẽ tự xử lý và hiển thị lỗi phù hợp
        // Chỉ log để debug
        console.error("Bad Request (400):", serverMessage || message);
      } else if (status === 403) {
        const translatedMessage = translateErrorMessage(serverMessage);
        notification.error({
          message: i18n.global.t("errors.accessDenied"),
