<template>
  <div class="so-view-modern">
    ...
    <a-card class="so-header-card" bordered>
      ...
      <flag-outlined style="color: #2563eb; font-size: 24px" />
      <span style="font-size: 20px; font-weight: 600">{{ $t("strategicObjectives") }}</span>
          </span>
...
async function submit() {
  ...
  try {
    if (!form.name){
      message?.error(`${$t("msgEnterStrategicObjectiveName")}`);
      return;
    }

    if (!form.perspectiveId){
      message?.error(`${$t("msgSelectPerspective")}`);
      return;
    }
