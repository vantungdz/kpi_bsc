<template>
  <div v-if="canAccessCreatePage">
    ...
    >
      ...
      <!-- Row 3: validity dates from header review cycle (read-only) -->
      <a-row :gutter="12">
        <a-col :span="8">
          <a-form-item
            class="textLabel"
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record, selected) => {
    if (form.value.assigned_user_id && isCopy.value) return;
...
const loadKpiTemplate = async (selectedId) => {
  ...
  try {
    ...
    if (kpiDetail) {
      ...
      if (!isCopy.value && kpiDetail.assignments && Array.isArray(kpiDetail.assignments)) {
        const newSelectedKeys = [];
        const newTargets = {};
        const keysToExpand = new Set();
        const hasSectionDepartmentKeys = new Set();

        kpiDetail.assignments.forEach(t => {
          let targetKey = "";
          if (t.assigned_to_department) {
            targetKey = `department - ${t.assigned_to_department}`;
            keysToExpand.add(targetKey);
          } else if (t.assigned_to_section) {
            targetKey = `section - ${t.assigned_to_section}`;
            if (t.section?.department.id) {
              hasSectionDepartmentKeys.add(`department - ${t.section?.department.id}`);
            }
          }

          if (targetKey) {
            newTargets[targetKey] = parseFloat(t.targetValue);
            targetValues.value[targetKey] = parseFloat(t.targetValue);
            newSelectedKeys.push(targetKey);
          }
        });
        selectedRowKeys.value = newSelectedKeys.filter(key => !hasSectionDepartmentKeys.has(key));
        form.value.targets = newTargets;

        // Update the ID list for the checkbox to display
        form.value.section_id = kpiDetail.assignments
          .filter(a => a.assigned_to_section)
          .map(a => a.assigned_to_section);

        form.value.department_id = kpiDetail.assignments
          .filter(a => a.assigned_to_department)
          .map(a => a.assigned_to_department);

        expandedRowKeys.value = Array.from(keysToExpand);
      }
    ...
    if (form.value.assigned_user_id && canAssignDirectlyToUser.value) {
      ...
      if (isOverAssigned.value) {
        assignmentError.value = $t("totalAssignedTargetExceedsKpiTarget", {
          totalAssigned: totalAssignedTarget.value,
          kpiTarget: overallTargetValue.value
        });
        throw new Error(assignmentError.value);
      }
    }
