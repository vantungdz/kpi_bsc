<template>
  <KpiDetailBase context-type="company">
    <template #context-content="slotProps">
      ...
      <!-- Department Assignments: Only show if KPI created by company -->
      <a-card
        :title="$t('departmentAssignments')"
        style="margin-top: 20px"
        v-if="slotProps.canAssignDepartment && slotProps.kpiDetailData?.created_by_type === 'company'"
      >
        <template #extra>
          <a-button
            type="primary"
      ...
      <!-- Section Assignments: Show if KPI created by company or department -->
      <a-card
        :title="$t('sectionAssignments')"
        style="margin-top: 20px"
        v-if="slotProps.canAssignSection && ['company', 'department'].includes(slotProps.kpiDetailData?.created_by_type)"
      >
        <template #extra>
          <a-button
            type="primary"
      ...
      <!-- Direct User Assignments: Show if KPI created by company, department, or section (not employee) -->
      <a-card
        :title="$t('directUserAssignments')"
        style="margin-top: 20px"
        v-if="slotProps.canAssignEmployees && ['company', 'department', 'section'].includes(slotProps.kpiDetailData?.created_by_type)"
      >
        <template #extra>
...
const companyOverviewDepartmentAssignments = (slotProps) => {
  const assignmentsArray = Array.isArray(slotProps.allUserAssignmentsForKpi)
    ? slotProps.allUserAssignmentsForKpi
    : [];

  // Filter: Show department assignments (with or without section), but exclude employee assignments
  // If an assignment has both department and section, we still show it in Department Assignments card
  // to indicate that the department is assigned, even though it also has a section assignment
  const filtered = assignmentsArray.filter(
    (a) =>
      a.assigned_to_department &&
      (!a.assigned_to_employee || a.assigned_to_employee === null)
  );

  // Remove duplicates: if there are multiple assignments for the same department
  // (one with section, one without), prefer the one without section for cleaner display
  const departmentMap = new Map();
  filtered.forEach((a) => {
    const deptId = a.assigned_to_department;
    const existing = departmentMap.get(deptId);
    if (!existing || (existing.assigned_to_section && !a.assigned_to_section)) {
      departmentMap.set(deptId, a);
    }
  });
...
const companyOverviewSectionAssignments = (slotProps) => {
  if (!slotProps.allUserAssignmentsForKpi) return [];
