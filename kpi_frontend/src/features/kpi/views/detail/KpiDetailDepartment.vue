          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'actual'">
                {{
                  record.latest_actual_value != null
              ...
              <template v-else-if="column.key === 'status'">
                <a-tag :color="getAssignmentStatusColor(record.latest_value_status)">
                  {{ getAssignmentStatusText(record.latest_value_status) }}
                </a-tag>
              </template>
        ...
        >
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'status'">
                <a-tag
                  :color="getAssignmentStatusColor(record.latest_value_status)"
                >
                  {{ getAssignmentStatusText(record.latest_value_status) }}
                </a-tag>
              </template>
...
const filteredAssignmentsForContextDepartment = (slotProps) => {
  const allAssignments = slotProps.allUserAssignmentsForKpi;
...
const shouldShowDepartmentSectionAssignmentCard = (slotProps) => {
  ...
  // Only show Section Assignments card if KPI created by company or department
  // KPIs created by section or employee should not show this card
  if (![SCOPES.COMPANY, SCOPES.DEPARTMENT].includes(kpi.created_by_type)) {
    return false;
  }

  if (hasDeptContext) {
    // Check if the department has sections
    ...
    // Only show this card if the department has sections
...
const filteredDirectUserAssignments = (slotProps) => {
  ...
  // Determine the department ID to filter by
  let filterDeptId = null;

  // If we have contextDepartmentId, use it (viewing in department context)
  if (slotProps.contextDepartmentId) {
    filterDeptId = slotProps.contextDepartmentId;
  }
  // Otherwise, if KPI was created by department, use that department ID
  else if (kpi?.created_by_type === "department") {
    filterDeptId = kpi?.created_by;
  }

  if (!kpi || !filterDeptId || !Array.isArray(allAssignments)) {
    return [];
  }
  ...
  return allAssignments.filter((assign) => {
    ...
    let employeeDepartmentId = assign.employee?.departmentId || assign.employee?.department_id;
    ...
    return (
      isUserAssignment &&
      employeeDepartmentId === filterDeptId &&
      !employeeHasSection
    );
  });
};
...
const shouldShowUserAssignmentsInDepartmentSectionsCard = (slotProps) => {
  ...
  // Only show User Assignments card if KPI created by company, department, or section
  // KPIs created by employee should not show this card
  if (
    !kpi ||
    ![SCOPES.COMPANY, SCOPES.DEPARTMENT, SCOPES.SECTION].includes(
      kpi.created_by_type,
    )
  ) {
    return false;
...
const shouldShowDirectUserAssignmentCard = (slotProps) => {
  ...
  // Only show Direct User Assignments card if KPI created by company, department, or section
  // KPIs created by employee should not show this card
  if (
    !kpi ||
    ![SCOPES.COMPANY, SCOPES.DEPARTMENT, SCOPES.SECTION].includes(
  ...
  // If we have a department context (contextDepartmentId)
  // Check if that department has sections
  if (slotProps.contextDepartmentId) {
    const deptId = slotProps.contextDepartmentId;
    const sections = slotProps.allSections.filter(
      (s) =>
    ...
    // Only show this card if the department has NO sections
    // If department has sections, Card 1 (UserAssignmentsInDepartmentSections) will handle it
    if (sections.length > 0) {
      return false;
    }
    // Department has no sections, continue to show this card
  } else {
    // No contextDepartmentId, check if KPI is created by department and has no sections
    if (kpi.created_by_type === "department") {
...
const getAssignmentStatusText = (status) => {
  if (!status) return $t("notSubmitted") || "Not Submitted";
  const valueStatusText = getKpiValueStatusText($t)[status];
  if (valueStatusText) return valueStatusText;
  return KpiDefinitionStatusText($t)[status] || status || "";
};

const getAssignmentStatusColor = (status) => {
  if (!status) return "default";
  if (KpiValueStatusColor[status]) return KpiValueStatusColor[status];
