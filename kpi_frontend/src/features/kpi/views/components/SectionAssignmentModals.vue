...
const handleSaveUserAssignment = async () => {
  ...
  // Client-side validation đã được loại bỏ - để backend xử lý validation
  // Backend sẽ trả về lỗi rõ ràng nếu target vượt quá giới hạn
  ...
  try {
    ...
    const errorMessage = getErrorDescription(err);
    // Chỉ hiển thị lỗi trong modal, không hiển thị notification để tránh trùng lặp
    userAssignmentSubmitError.value = errorMessage;
  } finally {
    submittingUserAssignment.value = false;
  }
};
