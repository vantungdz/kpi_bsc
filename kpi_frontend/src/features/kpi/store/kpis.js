...
const getters = {
  ...
  employeeKpiSummary: (state) => state.employeeKpiList?.summary ?? null,
  isLoadingEmployeeKpis: (state) => state.loadingEmployeeKpis,
  employeeKpiError: (state) => state.employeeKpiError,

  pendingApprovalList: (state) => state.pendingApprovalList,
  isLoadingPendingApproval: (state) => state.loadingPendingApproval,
  pendingApprovalError: (state) => state.pendingApprovalError,
};
...
const mutations = {
  ...
  SET_PENDING_APPROVAL_LIST(state, kpis) {
    state.pendingApprovalList = kpis || [];
  },
  SET_LOADING_PENDING_APPROVAL(state, isLoading) {
    state.loadingPendingApproval = isLoading;
  },
  SET_PENDING_APPROVAL_ERROR(state, error) {
    state.pendingApprovalError = error
      ? getTranslatedErrorMessage(error.response?.data?.message) ||
        error.message ||
        i18n.global.t("errors.unknownError")
      : null;
...
const actions = {
  ...
  async saveUserAssignments(
    ...
    try {
      ...
      // Lưu error message vào store để component có thể xử lý
      // Component sẽ tự hiển thị notification nếu cần
      const errorMsg =
  ...
  async fetchMyAssignments({ commit }, userIdOrFilters) {
    const filters =
      typeof userIdOrFilters === "object" && userIdOrFilters !== null
        ? userIdOrFilters
        : {};
    ...
    try {
      const response = await apiClient.get("/kpis/my-assignments", {
        params: filters,
      });
  ...
  async fetchKpisByEmployee({ commit }, { employeeId, filters = {} }) {
    ...
    try {
      ...
      commit("SET_EMPLOYEE_KPI_LIST", payload);
      return payload;
    } catch (error) {
  ...
  async bulkSubmitKpis({ commit }, kpiIds) {
    commit("SET_LOADING", true);
    commit("SET_ERROR", null);
    try {
      const response = await apiClient.post("/kpis/bulk-submit", { kpiIds });
      notification.success({
        message: i18n.global.t("kpisSubmittedSuccess"),
        description: `${response.data.success} KPI(s) submitted successfully`,
      });
      return response.data;
    } catch (error) {
      const errorMsg =
  ...
  async fetchPendingApprovalKpis({ commit }, params = {}) {
    commit("SET_LOADING_PENDING_APPROVAL", true);
    commit("SET_PENDING_APPROVAL_ERROR", null);
    try {
      const query = {};
      if (params.start_date) query.start_date = params.start_date;
      if (params.end_date) query.end_date = params.end_date;
      const response = await apiClient.get("/kpis/pending-approval", {
        params: query,
      });
      const kpis = Array.isArray(response.data) ? response.data : [];
      commit("SET_PENDING_APPROVAL_LIST", kpis);
      return kpis;
    } catch (error) {
      commit("SET_PENDING_APPROVAL_ERROR", error);
      commit("SET_PENDING_APPROVAL_LIST", []);
      throw error;
    } finally {
      commit("SET_LOADING_PENDING_APPROVAL", false);
    }
  },

  async approveKpi({ commit }, kpiId) {
    commit("SET_LOADING", true);
    commit("SET_ERROR", null);
    try {
      const response = await apiClient.patch(`/kpis/${kpiId}/approve`);
      notification.success({
        message: i18n.global.t("kpiApprovedSuccess"),
      });
      return response.data;
    } catch (error) {
      const errorMsg =
  ...
  async batchApproveKpis({ commit }, kpiIds) {
    commit("SET_LOADING", true);
    commit("SET_ERROR", null);
    try {
      const response = await apiClient.post("/kpis/approve", {
        kpiIds: Array.isArray(kpiIds) ? kpiIds : [kpiIds],
      });
      ...
      if (success && success.length > 0) {
        notification.success({
          message: i18n.global.t("kpiApprovedSuccess"),
          description: i18n.global.t("approvedCount", {
      ...
      if (failed && failed.length > 0) {
        const failedReasons = failed
          .map((f) => `KPI ${f.id}: ${f.reason}`)
          .join("\n");
        notification.warning({
          message: i18n.global.t("someKpisFailed"),
