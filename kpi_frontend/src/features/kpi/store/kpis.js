...
const mutations = {
  ...
  SET_SAVING_USER_ASSIGNMENT_LOADING(state, isLoading) { // Mutation mới
    state.isSavingUserAssignment = isLoading;
  },
  SET_SAVE_USER_ASSIGNMENT_ERROR(state, error) { // Mutation mới
    state.saveUserAssignmentError = error
      ? error.response?.data?.message || error.message || "Failed to save user assignment."
      : null;
  },
