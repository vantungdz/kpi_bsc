<template>
  <div class="role-manager-modern">
    ...
    <a-card class="role-header-card" bordered>
      ...
      <lock-outlined style="color: #1890ff; font-size: 24px" />
      <span style="font-size: 20px; font-weight: 600">{{ $t("roleList") }}</span>
          </span>
      ...
      <a-table
        ...
        rowKey="id"
        :loading="loading"
        class="role-table-modern"
        style="margin-top: 16px"
      >
        <template #actions="{ record }">
          <a-space>
            <a-button type="link" @click="goToRolePermissions(record)">
              <safety-certificate-outlined /> {{ $t("configureRolePermissions") }}
            </a-button>
    ...
    <a-modal
      :open="modalVisible"
      :title="editRole ? $t('editRole') : $t('addRole')"
      @ok="submitForm"
      @cancel="closeModal"
      :confirm-loading="saving"
      destroyOnClose
      class="role-modal-modern"
    >
      <a-form
        :model="roleForm"
        ref="roleFormRef"
...
const columns = computed(() => [
  { title: t("roleName"), dataIndex: "name", key: "name", width: 250 },
  { title: t("description"), dataIndex: "description", key: "description", width: 450 },
  {
    title: t("actions"),
    key: "actions",
...
const goToRolePermissions = (record) => {
  router.push({
    name: "RolePermissionManager",
    query: { roleId: String(record.id) },
  });
};
