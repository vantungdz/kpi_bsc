...
const actions = {
  ...
  async duplicateFormula({ dispatch, commit }, payload) {
    commit("SET_FORMULA_LOADING", true);
    commit("SET_FORMULA_ERROR", null);
    try {
      await apiClient.post("/kpi-formulas", payload);
      notification.success({
        message: i18n.global.t("formula.copyFormulaSuccess"),
      });
      await dispatch("fetchFormulas");
    } catch (error) {
      commit("SET_FORMULA_ERROR", error);
  ...
  async getFormulaById({ commit }, id) {
    commit("SET_FORMULA_ERROR", null);
    try {
      const res = await apiClient.get(`/kpi-formulas/${id}`);
      return res.data;
    } catch (error) {
      commit("SET_FORMULA_ERROR", error);
      throw error;
  ...
  async calculateScore({ commit }, { formulaId, body }) {
    commit("SET_FORMULA_ERROR", null);
    try {
      const res = await apiClient.post(
        `/kpi-formulas/${formulaId}/calculate-score`,
        body,
      );
      return res.data;
    } catch (error) {
