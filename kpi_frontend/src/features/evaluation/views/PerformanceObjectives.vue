<template>
  <div class="performance-objectives-page">
    <a-breadcrumb style="margin-bottom: 16px">
      ...
      <a-breadcrumb-item>{{ $t("performanceObjectivesTitle") }}</a-breadcrumb-item>
    </a-breadcrumb>
    ...
    <a-card :title="$t('performanceObjectivesEvaluation')" class="rounded-xl shadow-md">
      ...
      <a-row :gutter="16" style="margin-bottom: 20px">
        <a-col :xs="24" :sm="12" :md="8">
          <a-form-item :label="$t('selectDivision')">
            <a-select v-model:value="selectedDivision" :placeholder="$t('selectDivisionPlaceholder')"
              @change="handleDivisionChange" allow-clear>
        ...
        <a-col :xs="24" :sm="12" :md="8">
          <a-form-item :label="$t('selectEmployee')">
            <a-select v-model:value="selectedEmployee" :placeholder="$t('selectEmployeePlaceholder')"
              @change="handleEmployeeChange" :loading="isLoadingEmployees" :disabled="!selectedDivision" show-search
              :filter-option="employeeFilterOption" allow-clear>
              <a-select-option v-for="emp in employeesInDivision" :key="emp.id" :value="emp.id">
      ...
      <a-row class="mb-4" gutter="16" style="margin-bottom: 20px;">
        <a-col :span="8" v-if="selectedEmployeeDetails"><strong>Rank:</strong> {{ selectedEmployeeDetails.rank || ''
          }}</a-col>
      </a-row>
      ...
      <a-spin :spinning="isLoadingObjectives">
        ...
        <div v-else-if="selectedEmployee && !isLoadingObjectives && processedTableData.length === 0"
          class="empty-state">
            ...
            <template v-if="!record.isGroup">
              <template v-if="column.key === 'supervisorEvalScore'">
                <a-input-number
                  ...
                  @change="() => {
                    console.log('[TPL DEBUG] supervisorEvalScore @change. Current record.supervisorEvalScore:', record.supervisorEvalScore);
              ...
              <template v-else-if="column.key === 'weightedScoreSupervisor'">
                <span>{{ calcWeightedScore(record.supervisorEvalScore, record.weight) }}</span>
              </template>

              <!-- Target -->
              <template v-else-if="column.key === 'target'">
                <span>{{ formatNumberRef(record.target) }} ({{ record.unit }})</span>
              </template>

              <template v-else-if="column.key === 'actualResult'">
                <span>{{ formatNumberRef(record.actualResult) }} ({{ record.unit }})</span>
              </template>
        ...
        <div v-if="selectedEmployee && processedTableData.length > 0" class="mt-4">
          <a-row justify="space-between" align="middle">
            <a-col>
              <p><strong>{{ $t('totalWeightedScoreSupervisor') }}:</strong> {{ totalWeightedScoreSupervisor.toFixed(2)
                }}</p>
              <p><strong>{{ $t('averageScoreSupervisor') }}:</strong> {{ averageScoreSupervisor.toFixed(2) }}</p>
            </a-col>
            <a-col>
              <a-button type="primary" @click="saveEvaluation" :loading="isSaving">
                {{ $t('saveEvaluation') }}
              </a-button>
            </a-col>
          </a-row>
...
import { notification, Empty as AEmpty, Spin as ASpin, Breadcrumb as ABreadcrumb, BreadcrumbItem as ABreadcrumbItem, FormItem as AFormItem, InputNumber as AInputNumber, Textarea as ATextarea } from 'ant-design-vue';
...
const isLoadingObjectives = computed(() => store.getters['kpiEvaluations/isLoadingAssignedPerformanceObjectives']);
const isSaving = computed(() => store.getters['kpiEvaluations/isSavingPerformanceObjectiveEvaluation']);

const selectedEmployeeDetails = computed(() => {
  if (!selectedEmployee.value || !allEmployees.value.length) return null;
  return allEmployees.value.find(emp => emp.id === selectedEmployee.value);
});
...
const supervisorScoreFormatter = (value) => {
  console.log('[DEBUG supervisorScoreFormatter] Received raw value:', value, typeof value);
  if (value == null || String(value).trim() === '') {
    console.log('[DEBUG supervisorScoreFormatter] Value is null or empty, returning ""');
    return '';
  }
  const num = Number(value);
  if (isNaN(num)) {
    console.log('[DEBUG supervisorScoreFormatter] Value is NaN after Number() conversion. Original value:', value, 'Returning original.');
    return String(value); // Hoặc return '' nếu muốn hiển thị giá trị không hợp lệ là rỗng
  }
  const formatted = new Intl.NumberFormat('en-US').format(num);
  console.log('[DEBUG supervisorScoreFormatter] Returned formatted value:', formatted);
  return formatted;
};

const supervisorScoreParser = (displayValue) => {
  console.log('[DEBUG supervisorScoreParser] Received display value:', displayValue, typeof displayValue);
  if (displayValue == null || String(displayValue).trim() === '') {
     console.log('[DEBUG supervisorScoreParser] Display value is null or empty, returning "" for AInputNumber to parse as null.');
  ...
  console.log('[DEBUG supervisorScoreParser] Returned cleaned string for AInputNumber to convert:', cleanedValue);
...
const columns = computed(() => [
  {
    ...
    customRender: ({ record, text }) => {
      if (record.isGroup) { // This is the group header row
        return {
          children: h('strong', null, record.item),
          props: {
            colSpan: numberOfColumns,
            style: { backgroundColor: '#fafafa', textAlign: 'left', paddingLeft: '16px', paddingTop: '12px', paddingBottom: '12px' }
          },
        };
      }
  ...
  },
  {
    ...
    customRender: ({ record }) => {
      ...
      const targetValue = typeof record.target === 'string' ? parseFloat(record.target) : record.target;
      const formattedTarget = formatNumber(targetValue);
      return record.unit ? `${formattedTarget} (${record.unit})` : formattedTarget || '';
    },
  },
  {
    ...
    customRender: ({ record }) => {
      if (record.isGroup) return { props: { colSpan: 0 } };
      const actualResultValue = typeof record.actualResult === 'string' ? parseFloat(record.actualResult) : record.actualResult;
      const formattedActualResult = formatNumber(actualResultValue);
      return record.unit ? `${formattedActualResult} (${record.unit})` : formattedActualResult || '';
    },
  },
  {
    title: $t('weight'),
...
const processedTableData = computed(() => {
  ...
  const grouped = objectivesData.value.reduce((acc, curr) => {
    ...
    acc[aspect].push({
      ...curr,
      supervisorEvalScore: curr.supervisorEvalScore === undefined ? null : curr.supervisorEvalScore,
...
const averageScoreSupervisor = computed(() => {
  const scoredItems = objectivesData.value.filter(item => typeof item.supervisorEvalScore === 'number' && typeof item.weight === 'number' && item.weight > 0);
  const totalWeightOfScoredItems = scoredItems.reduce((sum, item) => sum + item.weight, 0);
...
const handleDivisionChange = (value) => {
  ...
  if (value) {
    ...
    employeesInDivision.value = allEmployees.value.filter(emp => emp.departmentId === value);
  } else {
    employeesInDivision.value = [];
  }
};
...
const fetchObjectives = async () => {
  if (selectedEmployee.value) { // Removed selectedCycle condition
    try {
      const fetchedData = await store.dispatch('kpiEvaluations/fetchAssignedPerformanceObjectives', { // Corrected dispatch path
        employeeId: selectedEmployee.value,
      });
      
      objectivesData.value = (fetchedData || []).map(item => ({
        ...item,
        supervisorEvalScore: item.supervisorEvalScore === undefined ? null : item.supervisorEvalScore,
        note: item.note === undefined ? '' : item.note,
        unit: item.unit || '', // Ensure unit is mapped from fetched data
      }));
    } catch (error) {
      notification.error({ message: $t('errorFetchingObjectives'), description: error.message });
      objectivesData.value = [];
    }
  } else {
    objectivesData.value = [];
...
const handleEvaluationChange = (objectiveKey) => {
  ...
  const objectiveInProcessedData = processedTableData.value.find(item => item.key === objectiveKey);
  if (objectiveInProcessedData) {
    // If objectiveKey is item.id (for non-group items), this lookup is direct.
    const originalObjective = objectivesData.value.find(item => item.id === objectiveKey);
    if (originalObjective) {
      originalObjective.supervisorEvalScore = objectiveInProcessedData.supervisorEvalScore;
...
const saveEvaluation = async () => {
  if (!selectedEmployee.value) { // Removed selectedCycle condition
    notification.error({ message: $t('error'), description: $t('pleaseSelectEmployee') }); // Updated error message
    return;
  }
  ...
  try {
    await store.dispatch('kpiEvaluations/savePerformanceObjectiveEvaluation', evaluationPayload); // Corrected dispatch path
  } catch (error) {
    notification.error({ message: $t('errorSavingEvaluation'), description: error.message });
  }
};
...
watch(selectedDivision, (newVal) => {
  if (newVal) {
    employeesInDivision.value = allEmployees.value.filter(emp => emp.departmentId === newVal);
  } else {
    employeesInDivision.value = [];
  }
  selectedEmployee.value = null; 
