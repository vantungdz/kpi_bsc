<template>
  <div class="review-cycle-create-modern">
    ...
    <a-card class="cycle-header-card" bordered>
      ...
      <calendar-outlined style="color: #2563eb; font-size: 24px" />
      <span style="font-size: 20px; font-weight: 600">{{ $t("reviewCycle.createTitle") }}</span>
          </span>
        </div>
      </template>
      <a-form
...
/** Keep AppHeader review-cycle dropdown in sync with this page. */
function refreshGlobalReviewCycles() {
  return store.dispatch("reviewCycle/fetchCycles");
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await fetchCycles();
    await refreshGlobalReviewCycles();
  } catch (e) {
    errorMessage.value =
