    ...
    >
      <template #title>
        <span style="display: flex; align-items: center; gap: 10px">
          <solution-outlined style="color: #1890ff; font-size: 24px" />
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'workflow'">
            <div class="workflow-cell">
              <a-tag :color="getReviewWorkflow(record.status).tagColor">
                {{ getReviewWorkflow(record.status).currentStep }}
              </a-tag>
              <div class="workflow-next">
                {{ getReviewWorkflow(record.status).nextAction }}
              </div>
            </div>
          </template>
          <template v-else-if="column.key === 'actions'">
            <div style="text-align: center">
              <a-button
                v-if="
                  record.status === 'EMPLOYEE_FEEDBACK' ||
                  record.status === 'MANAGER_REVIEWED'
                "
                type="primary"
                size="small"
...
const getReviewWorkflow = (status) => getReviewWorkflowSummary(status, t);
...
const getStatusColor = (status, record) => {
  ...
  if (status === "MANAGER_REVIEWED" || status === "COMPLETED") return "green";
...
const fetchCycles = async () => {
  try {
    ...
    if (route.query.cycle) {
      selectedCycle.value = Number(route.query.cycle);
      await nextTick();
      fetchMyKpis();
      return;
    }
...
const tryHandleReviewQuery = () => {
  const reviewId = Number(route.query.reviewId);
  if (!reviewId || !kpis.value.length) return;

  const reviewRecord = kpis.value.find((record) => Number(record.id) === reviewId);
  if (!reviewRecord) return;

  if (
    route.query.action === "feedback" &&
    (reviewRecord.status === "EMPLOYEE_FEEDBACK" ||
      reviewRecord.status === "MANAGER_REVIEWED")
  ) {
    const reviewIndex = kpis.value.findIndex((record) => Number(record.id) === reviewId);
    openFeedbackModal(reviewRecord, reviewIndex);
    return;
  }

  if (route.query.action === "detail" || route.query.action === "focus") {
    openDetailModal(reviewRecord);
  }
};
...
const onCycleChange = (val) => {
  ...
  pushReviewCycleToGlobalStore(store, val ?? null);
  fetchMyKpis();
};
...
watch(
  ...
  (newKpis) => {
    newKpis.forEach((k) => {
      if (
        k.status === "EMPLOYEE_FEEDBACK" ||
        k.status === "MANAGER_REVIEWED"
      ) {
