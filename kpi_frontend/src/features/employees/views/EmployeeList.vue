        ...
        <template v-else-if="column.key === 'actions'">
          <a-space>
            ...
            <a-button
              ...
              v-if="canDeleteEmployee && !isCurrentUser(record)"
            >
    ...
    >
      <a-descriptions v-if="detailEmployee" bordered column="1" size="middle">
        ...
        <a-descriptions-item :label="$t('fullName')">
          <idcard-outlined style="margin-right: 4px" />{{ $getFullName(detailEmployee) }}
        </a-descriptions-item>
...
const currentUser = computed(() => store.getters["auth/user"] || null);

// Check if employee is the current logged in user
const isCurrentUser = (employee) => {
  if (!currentUser.value || !employee) return false;
...
const departmentList = computed(() => {
  const depts = store.getters["departments/departmentList"] || [];
  return depts;
});
...
const openAddModal = async () => {
  // Fetch latest departments and sections to ensure dropdowns have new data
  await Promise.all([
...
const openEditModal = async (employee) => {
  ...
  if (employee.department?.id) {
    ...
    if (!sections || !sections.length) {
      ...
      // Force refresh sections for this department to get new sections
      await store.dispatch(
        "sections/fetchSectionsByDepartment",
        employee.department.id,
      );
...
const handleAddEditEmployee = async () => {
  ...
  try {
    ...
    let errMess = e.message || $t("saveEmployeeErrorDefault");
    if (e.response) {
      errMess = e.response.data.message;
      if (Array.isArray(errMess)) {
        errorMessage.value = errMess.join(', ');
      } else {
        errorMessage.value = errMess || 'An error occurred';
      }
    }
    notification.error({
      message: $t("saveEmployeeError"),
      description: errMess || $t("saveEmployeeErrorDefault"),
    });
  } finally {
...
const handleResetPassword = async () => {
  if (!resetPasswordValue.value) {
    message.error($t("personalGoal.msgNewPassword"));
    return;
  }
