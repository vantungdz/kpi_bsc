    ...
    <div class="header-bar">
      <div class="header-left">
        <h2 class="page-title">
          ...
          {{ $t("rolePermissionManagement") }}</h2>
      </div>
      <div class="header-controls">
        <a-form-item :label="$t('searchPermission')" class="filter-item">
          <a-input
        ...
        <a-form-item label=" " :colon="false" class="filter-item">
          <a-button
            type="primary"
            @click="saveAll"
            :loading="loading"
    ...
    <div class="permission-group-list">
      ...
      >
        <div class="permission-group-title">
          <div class="permission-group-title-left">
            <i class="fa fa-layer-group group-icon"></i>
            <span>{{ group.label }}</span>
            <span class="group-count">
              ({{ group.permissions.length
              }}<template v-if="searchText.trim()"
                >/{{ fullGroupPermissionCount(group) }}</template
              >)
            </span>
          </div>
          <div class="permission-group-title-master">
            <a-tooltip :title="$t('permissionBlock.toggleAllTooltip')">
              <span class="master-toggle-label">{{ $t("permissionBlock.toggleAll") }}</span>
            </a-tooltip>
            <a-switch
              :checked="isGroupFullyOn(group)"
              @change="(checked) => onToggleGroupAll(group, checked)"
            />
          </div>
        </div>
        <div class="permission-group-grid">
          ...
          >
            <a-switch
              ...
              :disabled="
                permission.id === LOCKED_PERMISSION_ID &&
                currentRoleId === LOCKED_ROLE_ID
              "
              @change="
                (checked) =>
...
function roleIdsMatch(a, b) {
  if (a == null || b == null) return false;
  return Number(a) === Number(b);
}

function syncCurrentRoleFromQuery() {
  const list = rolesWithPermissions.value;
  if (!list?.length) return;
  const q = route.query.roleId;
  if (q != null && q !== "") {
    const idNum = Number(q);
    if (!Number.isNaN(idNum)) {
      const match = list.find((r) => roleIdsMatch(r.id, idNum));
      if (match) {
        currentRoleId.value = match.id;
        return;
      }
    }
  }
  if (!list.some((r) => roleIdsMatch(r.id, currentRoleId.value))) {
...
const onTogglePermission = (record, permissionId, checked) => {
  if (checked) {
    ...
    if (permissionId === 43 && currentRoleId.value === 1) {
      return;
    }
...
const LOCKED_PERMISSION_ID = 43;
const LOCKED_ROLE_ID = 1;

function getBulkPermissionList(group) {
  const key = group.actionKey;
  const full = permissionGroups.value.find((g) => g.actionKey === key);
  return full?.permissions?.length ? full.permissions : group.permissions;
}

function fullGroupPermissionCount(group) {
  return getBulkPermissionList(group).length;
}

function isGroupFullyOn(group) {
  const role = currentRole.value;
  const list = getBulkPermissionList(group);
  if (!list.length) return false;
  return list.every((p) => role.permissionIds.includes(p.id));
}

function onToggleGroupAll(group, checked) {
  const role = currentRole.value;
  const list = getBulkPermissionList(group);
  ...
  if (checked) {
    list.forEach((p) => {
      if (!role.permissionIds.includes(p.id)) {
        role.permissionIds.push(p.id);
      }
    });
  } else {
    role.permissionIds = role.permissionIds.filter((id) => {
      if (!idSet.has(id)) return true;
      if (id === LOCKED_PERMISSION_ID && currentRoleId.value === LOCKED_ROLE_ID) {
        return true;
      }
      return false;
    });
...
const currentRole = computed(
  () =>
    rolesWithPermissions.value.find((r) =>
      roleIdsMatch(r.id, currentRoleId.value),
    ) ||
...
watch(
  ...
  (val) => {
    ...
    if (!val.some((r) => roleIdsMatch(r.id, currentRoleId.value))) {
      syncCurrentRoleFromQuery();
    }
  },
  { immediate: true },
...
.permission-group-title-left {
  display: flex;
  align-items: center;
  gap: 10px;
  min-width: 0;
}
.permission-group-title-master {
  display: flex;
  align-items: center;
  gap: 8px;
  flex-shrink: 0;
