<template>
  <div class="user-activity-statistics-container">
    <LoadingOverlay
      :visible="isLoadingTopPendingApprovers || isLoadingKpiSubmissionStats"
    />
    ...
    <h1 style="font-size: 20px; font-weight: 600"><user-switch-outlined style="margin-right: 8px; color: #722ed1; font-size: 24px" /> {{ $t("userActivityStatisticsTitle") }}</h1>
    ...
    <a-card :title="$t('topPendingApprovers')" class="dashboard-card">
      <a-row :gutter="[16, 16]">
        <a-col v-if="topPendingApproversError" :span="24">
          <a-alert
            :message="$t('error')"
            :description="topPendingApproversError"
            type="error"
            show-icon
          />
        </a-col>
        <template v-if="!topPendingApproversError">
          <a-col :span="24">
            <a-skeleton
              :loading="isLoadingTopPendingApprovers"
              active
              avatar
              :paragraph="{ rows: 5 }"
              :title="false"
            >
              <a-list
                bordered
                :dataSource="topPendingApproversStats"
                v-if="topPendingApproversStats.length > 0"
                ...
                <template #renderItem="{ item }">
                  <a-list-item>
                    <div class="list-item-content approver-item">
                      <div
                        class="item-header expandable"
                        @click="toggleExpand(item.approverId, 'approvers')"
                      >
                        <span class="expand-icon">
                          <down-outlined
                            v-if="isExpanded(item.approverId, 'approvers')"
                          />
                        ...
                        <span class="item-title">
                          {{ item.approverName }}
                          <a-tag
                            :color="getApproverTypeColor(item.approverType)"
              ...
              <div
                v-else-if="
                  !isLoadingTopPendingApprovers &&
                  topPendingApproversStats.length === 0
                "
                class="empty-list"
              >
    ...
    >
      <div class="chart-container">
        <bar-chart
          :chart-data="topPendingApproversChartData"
          :chart-options="topPendingApproversChartOptions"
          style="height: 350px"
        />
      </div>
    </a-card>
    ...
    <a-card
      :title="$t('kpiSubmissionStats')"
      class="dashboard-card card-submission-stats"
    >
      <a-row :gutter="[16, 16]" style="margin-bottom: 16px">
        <a-col :xs="24" :sm="12" :md="12">
          <label>{{ $t("entityType") }}: </label>
          <a-radio-group
            v-model:value="kpiSubmissionFilters.entityType"
            button-style="solid"
            size="small"
          >
            <a-radio-button
        ...
        <a-col :xs="24" :sm="12" :md="12">
          ...
          <a-radio-group
            v-model:value="kpiSubmissionFilters.orderBy"
            button-style="solid"
            size="small"
          >
            <a-radio-button
        ...
        <a-col :xs="24" :sm="24" :md="8">
          ...
          <a-select
            v-model:value="kpiSubmissionFilters.days"
            style="width: 100%"
            size="small"
          >
            <a-select-option
        ...
        <a-col :xs="24" :sm="12" :md="8">
          <label>{{ $t("top") }}: </label>
          <a-input-number
            v-model:value="kpiSubmissionFilters.limit"
            :min="1"
            :max="50"
            style="width: 100%"
            size="small"
        ...
        <a-col :xs="24" :sm="12" :md="8">
          ...
          <a-input-number
            v-model:value="kpiSubmissionFilters.recentKpisLimit"
            :min="0"
            :max="10"
            style="width: 100%"
            size="small"
      ...
      <a-row :gutter="[16, 16]">
        <a-col v-if="kpiSubmissionStatsError" :span="24">
          <a-alert
            :message="$t('error')"
            :description="kpiSubmissionStatsError"
            type="error"
            show-icon
          />
        </a-col>
        <template v-if="!kpiSubmissionStatsError">
          <a-col :span="24">
            <a-skeleton
              :loading="isLoadingKpiSubmissionStats"
              active
              avatar
              :paragraph="{ rows: 5 }"
            >
                  ...
                  <a-list-item>
                    <div class="list-item-content">
                      <div
                        class="item-header"
                        :class="{
                          expandable: kpiSubmissionFilters.recentKpisLimit > 0,
                        }"
                        @click="
                          kpiSubmissionFilters.recentKpisLimit > 0
                            ? toggleKpiSubmissionExpand(item.id || index)
                            : null
                        "
                      >
                        <span
                        ...
                        <span
                          class="item-id"
                          v-if="
                            kpiSubmissionFilters.entityType !== 'user' &&
                            item.id
                          "
                        >
                          (ID: {{ item.id }})</span
                        ...
                        <span class="item-count"
                          ...
                          {{ $t("submissionCount") }}</span
                        >
                      </div>
                      <div
                        v-if="
                          kpiSubmissionFilters.recentKpisLimit > 0 &&
                          isKpiSubmissionItemExpanded(item.id || index)
                        "
                        class="expanded-details"
                      >
                        <div
                          v-if="
                            item.recentSubmittedKpis &&
                            item.recentSubmittedKpis.length > 0
                          "
                          class="recent-kpis-list"
                        >
                          <small>{{
                            $t("recentSubmittedKpis", {
                              count: item.recentSubmittedKpis.length,
                            })
                          }}</small>
                          <ul>
                            <li
                              v-for="kpi_detail in item.recentSubmittedKpis"
              ...
              <div
                v-else-if="
                  !isLoadingKpiSubmissionStats &&
                  kpiSubmissionStats.length === 0
                "
                class="empty-list"
              >
    ...
    >
      <div class="chart-container">
        <bar-chart
          :chart-data="kpiSubmissionChartData"
          :chart-options="kpiSubmissionChartOptions"
          style="height: 350px"
        />
      </div>
    </a-card>
...
import { DownOutlined, RightOutlined, UserSwitchOutlined } from "@ant-design/icons-vue";

const { t: $t } = useI18n();
const store = useStore();
const router = useRouter();

const topPendingApproversStats = computed(
  () => store.getters["dashboard/getTopPendingApproversStats"]
);
const isLoadingTopPendingApprovers = computed(
  () => store.getters["loading/isLoading"]
);
const topPendingApproversError = computed(
  () => store.getters["dashboard/getTopPendingApproversError"]
);
...
const kpiSubmissionStats = computed(
  () => store.getters["dashboard/getKpiSubmissionStats"]
);
const isLoadingKpiSubmissionStats = computed(
  () => store.getters["loading/isLoading"]
);
const kpiSubmissionStatsError = computed(
  () => store.getters["dashboard/getKpiSubmissionStatsError"]
);

const toggleKpiSubmissionExpand = (itemId) => {
  toggleExpand(itemId, "submission");
};
const isKpiSubmissionItemExpanded = (itemId) =>
  isExpanded(itemId, "submission");
...
function fetchKpiSubmissionData() {
  store.dispatch("dashboard/fetchKpiSubmissionStats", {
...
const topPendingApproversChartData = computed(() => {
  const labels = topPendingApproversStats.value.map(
    (item) => `${item.approverName} (${formatApproverType(item.approverType)})`
  );
...
const kpiSubmissionChartData = computed(() => {
  const labels = kpiSubmissionStats.value.map(
    (item) => item.name || $t("undefined")
  );
  ...
  return {
    ...
    datasets: [
      {
        label: $t("submissionCountLabel", {
          orderBy:
            kpiSubmissionFilters.orderBy === "most" ? $t("most") : $t("least"),
        }),
        backgroundColor: backgroundColors.slice(0, data.length),
        borderColor: borderColors.slice(0, data.length),
        borderWidth: 1,
...
const kpiSubmissionChartOptions = computed(() => ({
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: $t("kpiSubmissionChartTitle", {
        entityType:
          kpiSubmissionFilters.entityType === "user"
            ? $t("user")
            : kpiSubmissionFilters.entityType === "section"
              ? $t("section")
              : $t("department"),
      }),
      font: { size: 16 },
  ...
  scales: {
    x: {
      beginAtZero: true,
      ticks: { precision: 0 },
      title: { display: true, text: $t("submissionCount") },
    },
    y: { ticks: { autoSkip: false } },
  },
}));
