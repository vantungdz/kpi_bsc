            ...
            <div v-if="!loadingError">
                ...
                <a-card title="Các chỉ số hiệu suất chính" class="dashboard-card performance-summary-card">
                    <a-row :gutter="[16, 24]" align="middle">
                        ...
                        <a-col :xs="24" :sm="24" :md="10" :lg="8">
                            <div class="chart-container main-overview-chart"
                                v-if="!isLoadingOverview && kpiOverviewStats && (kpiOverviewStats.achievedCount > 0 || kpiOverviewStats.notAchievedCount > 0)">
                                <pie-chart :chart-data="kpiAchievementChartData"
                ...
                <a-card title="Trạng thái KPI theo vai trò" class="dashboard-card card-style-role-performance">
                    ...
                    <a-table
                        ...
                        :columns="performanceByRoleColumns" :data-source="kpiOverviewStats.performanceByRole"
                        :pagination="false" size="small" bordered row-key="roleId">
                        <template #bodyCell="{ column, record }">
                            <template v-if="column.key === 'achievedRate'">
                                <a-progress :percent="record.achievedRate"
                                    :status="record.achievedRate >= 75 ? 'success' : record.achievedRate >= 50 ? 'normal' : 'exception'"
                                    size="small" />
                            </template>
                            ...
                            <template v-else-if="column.key === 'notAchievedCount'">
                                <span style="color: #cf1322">{{ record.notAchievedCount }}</span>
                            </template>
                        </template>
                    </a-table>
                </a-card>
