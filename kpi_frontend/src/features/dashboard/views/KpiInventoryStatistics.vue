<template>
  <div class="kpi-inventory-statistics-container">
    ...
    <h1 style="font-size: 20px; font-weight: 600"><database-outlined style="margin-right: 8px; color: #13c2c2; font-size: 24px" /> {{ $t("kpiInventoryOverview") }}</h1>
    ...
    <a-spin :spinning="isLoading" :tip="$t('loadingData')">
      ...
      <div v-if="!loadingError">
        ...
        <a-card
          ...
          class="dashboard-card summary-metrics-card card-style-summary"
        >
          <a-row :gutter="[16, 16]">
              ...
              <div class="metric-item">
                ...
                >
                  <a-statistic
                    ...
                    :value="inventoryStats?.totalKpiDefinitions ?? 0"
              ...
              <div class="metric-item">
                ...
                >
                  <a-statistic
                    ...
                    :value="inventoryStats?.totalKpiAssignments ?? 0"
                    class="statistic-value"
                  />
                </a-skeleton>
              </div>
        ...
        <a-card
          :title="$t('assignmentsByDepartment')"
          class="dashboard-card card-style-department"
        >
          <a-skeleton :loading="isLoading" active :paragraph="{ rows: 5 }" />
          <div
            v-if="
              !isLoading && !inventoryStats?.assignmentsByDepartment?.length
            "
            class="empty-list"
          >
            {{ $t("noDepartmentData") }}
          ...
          <div
            ...
            v-if="!isLoading && inventoryStats?.assignmentsByDepartment?.length"
          >
            <bar-chart
              :chart-data="assignmentsByDepartmentChartData"
              :chart-options="assignmentsByDepartmentChartOptions"
              style="height: 350px"
            />
          </div>
        </a-card>
        ...
        <a-card
          :title="$t('assignmentsByStatus')"
          class="dashboard-card card-style-status"
        >
          <a-skeleton :loading="isLoading" active :paragraph="{ rows: 5 }" />
          <div
            v-if="!isLoading && !inventoryStats?.assignmentsByStatus?.length"
            class="empty-list"
          >
            {{ $t("noStatusData") }}
          </div>
          ...
          >
            <pie-chart
              :chart-data="assignmentsByStatusChartData"
              :chart-options="assignmentsByStatusChartOptions"
              style="height: 300px"
            />
          </div>
        </a-card>
      ...
      <a-empty
        v-if="!isLoading && !inventoryStats && !loadingError"
        :description="$t('noKpiInventoryData')"
      />
    </a-spin>
  </div>
</template>
...
const loadingError = computed(
  () => store.getters["dashboard/getKpiInventoryError"]
);
const inventoryStats = computed(
  () => store.getters["dashboard/getKpiInventoryStats"]
);
...
const assignmentsByDepartmentChartOptions = computed(() => ({
  ...
  plugins: {
    ...
    title: { display: true, text: $t("kpiAssignmentsByDepartment") },
  },
  scales: {
    x: {
      beginAtZero: true,
      ticks: { precision: 0 },
      title: { display: true, text: $t("kpiAssignmentsCount") },
    },
...
const assignmentsByStatusChartData = computed(() => {
  ...
  const labels = data.map((item) => $t(`status_chart.${item.status}`));
  const counts = data.map((item) => item.count);
...
const assignmentsByStatusChartOptions = computed(() => ({
  ...
  plugins: {
    legend: { position: "top" },
    title: { display: true, text: $t("kpiAssignmentsByStatus") },
  },
}));
