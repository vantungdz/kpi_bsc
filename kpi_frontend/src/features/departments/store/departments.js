...
const actions = {
  ...
  /**
   * Update sort order for multiple departments.
   */
  async updateDepartmentOrder({ dispatch, commit }, departments) {
    try {
      const response = await apiClient.patch("/departments/update-order", {
        departments,
      });
      ...
      console.error("Error updating department order:", error.response || error);
      const msg =
        error?.response?.data?.message ||
        error.message ||
        "Failed to update department order";
      commit("SET_ERROR", msg);
      throw new Error(msg);
    }
  },
