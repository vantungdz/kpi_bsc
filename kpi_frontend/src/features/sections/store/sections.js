...
const actions = {
  ...
  async updateSection({ dispatch }, { id, data }) {
    ...
    try {
      ...
      // Only fetch sections if update was successful (no warning)
   ...
   */
  async updateSectionOrder({ dispatch, commit }, sections) {
    try {
      const response = await apiClient.patch("/sections/update-order", {
        sections,
      });
      ...
      console.error("Error updating section order:", error.response || error);
      const msg =
        error?.response?.data?.message ||
        error.message ||
