  ...
  <div class="kpi-value-approval-list-page">
    <a-card :title="cardTitle" :bordered="false">
      <a-spin :spinning="isLoadingPending" tip="Đang tải danh sách...">
        ...
        <a-table v-if="!loadingError && pendingItems.length > 0" :columns="columns" :data-source="pendingItems"
          :row-key="'id'" bordered size="small" :scroll="{ x: 'max-content' }">
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'target'">
              <span
                v-if="record.kpiAssignment?.targetValue !== null && record.kpiAssignment?.targetValue !== undefined">
            ...
            <template v-else-if="column.key === 'actions'">
              <a-space>
                <a-button type="link" size="small" @click="openDetailModal(record)" title="Xem Chi tiết & Lịch sử">
                ...
                <a-button type="primary" size="small" @click="handleApprove(record.id)"
                  ...
                  :disabled="isProcessing && currentActionItemId !== record.id" title="Phê duyệt">
                  <check-outlined /> Approve
                </a-button>
                <a-button danger size="small" @click="openRejectModal(record)" :disabled="isProcessing" title="Từ chối">
    ...
    <a-modal :open="isRejectModalVisible" title="Lý do từ chối" @ok="handleReject" @cancel="closeRejectModal"
      :confirm-loading="isProcessing && currentActionItemId === itemToReject?.id" ok-text="Xác nhận Từ chối"
      cancel-text="Hủy" :maskClosable="false" destroyOnClose>
      <a-alert v-if="rejectError" :message="rejectError" type="error" show-icon style="margin-bottom: 10px" />
      <p v-if="itemToReject" style="margin-bottom: 10px;">
        ...
        <strong>{{ itemToReject.kpiAssignment?.employee?.first_name }} {{
      ...
      <a-form-item label="Lý do (bắt buộc):" :validate-status="rejectError ? 'error' : ''" :help="rejectError">
        <a-textarea v-model:value="rejectionReason" placeholder="Nhập lý do từ chối..." :rows="4" />
      </a-form-item>
    </a-modal>

    <a-modal :open="isDetailModalVisible" title="Chi tiết và Lịch sử Cập nhật KPI" @cancel="closeDetailModal"
      ...
      <a-spin :spinning="!selectedKpiValue" tip="Đang tải chi tiết...">
        <div v-if="selectedKpiValue">
          <a-descriptions bordered size="small" :column="2">
            <a-descriptions-item label="Tên KPI">{{ selectedKpiValue.kpiAssignment?.kpi?.name || ''
              }}</a-descriptions-item>
            <a-descriptions-item label="Nhân viên">{{ `${selectedKpiValue.kpiAssignment?.employee?.first_name || ''}
              ${selectedKpiValue.kpiAssignment?.employee?.last_name || ''}` }}</a-descriptions-item>
            <a-descriptions-item label="Target Giao">{{ selectedKpiValue.kpiAssignment?.targetValue?.toLocaleString() }}
              {{ selectedKpiValue.kpiAssignment?.kpi?.unit }}</a-descriptions-item>
            <a-descriptions-item label="Đơn vị Tính">{{ selectedKpiValue.kpiAssignment?.kpi?.unit || ''
              }}</a-descriptions-item>
          </a-descriptions>

          <a-descriptions title="Chi tiết Lần Submit Hiện tại" bordered size="small" :column="1"
            ...
            <a-descriptions-item label="Giá trị Nộp">{{ selectedKpiValue.value?.toLocaleString() ?? ''
              }}</a-descriptions-item>
            <a-descriptions-item label="Thời gian Nộp/Cập nhật">{{ formatDate(selectedKpiValue.timestamp ||
              selectedKpiValue.updated_at) }}</a-descriptions-item>
            <a-descriptions-item label="Ghi chú (Notes)">{{ selectedKpiValue.notes || 'Không có'
              }}</a-descriptions-item>

            <a-descriptions-item label="Chi tiết Dự án/Công việc">
              <div
                v-if="Array.isArray(selectedKpiValue.project_details) && selectedKpiValue.project_details.length > 0">
                <a-table :columns="projectDetailsColumns" :data-source="selectedKpiValue.project_details"
                  :pagination="false" size="small" bordered row-key="name" style="max-width: 500px;">
                  <template #bodyCell="{ column, record }">
            ...
            <a-descriptions-item label="Lý do Từ chối (nếu có)" v-if="selectedKpiValue.rejection_reason">
              <span style="color: red;">{{ selectedKpiValue.rejection_reason }}</span>
            </a-descriptions-item>
          </a-descriptions>

          <h3 style="margin-top: 20px; margin-bottom: 10px;">Lịch sử Cập nhật/Phê duyệt</h3>
              ...
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'noteOrReason'">
                  <a-tooltip v-if="record.reason" :title="record.reason">
                    <span style="color: red;">Lý do: {{ truncateText(record.reason, 70) }}</span>
                  </a-tooltip>
                ...
                <template v-else-if="column.key === 'changed_by'">
                  ...
                  <span v-else-if="record.changed_by"> ID: {{ record.changed_by }}
                  </span>
                  <span v-else>N/A</span>
                </template>
              </template>
...
import { KpiValueStatus, KpiValueStatusText, KpiValueStatusColor } from '../constants/kpiStatus'; 
...
const columns = ref([ 
  ...
  { title: 'Ngày Submit', dataIndex: 'timestamp', key: 'submittedAt', width: 140 }, 
...
const historyColumns = ref([
  ...
  { title: 'Giá trị', dataIndex: 'value', key: 'value', align: 'right', width: 100 },
  { title: 'Status Trước', key: 'status_before', align: 'center', width: 100 },
  { title: 'Status Sau', key: 'status_after', align: 'center', width: 100 },
...
const projectDetailsColumns = ref([
  { title: 'Tên Dự án/Công việc', dataIndex: 'name', key: 'name', ellipsis: true }, 
  { title: 'Giá trị', dataIndex: 'value', key: 'value', align: 'right', width: 100 }, 
]);
