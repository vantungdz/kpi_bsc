  ...
  <div class="kpi-personal-list-page">
    <a-card>
      ...
      <div style="margin-bottom: 20px">
        ...
        />
        <a-alert
          v-else-if="myAssignments.length === 0 && !loadingMyAssignments"
          message="You have no assigned KPIs."
          type="warning"
          show-icon
        />
        <a-alert
          ...
          type="success"
          closable
          @close="submissionSuccessMessage = null"
          show-icon
        />
      </div>
      ...
      <div v-if="!loadingMyAssignments && hasKpis">
        ...
        >
          <a-collapse-panel
            ...
            :key="perspectiveId"
            :header="`${kpiList[0].perspective?.id || '?'}. ${
              kpiList[0].perspective?.name || 'Uncategorized'
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'name'">
                  <a-tooltip
                    :title="`KPI ID: ${record.kpi_id} | Assignment ID: ${record.assignment_id}`"
                  >
                    {{ record.name }}
                  </a-tooltip>
                </template>
                ...
                <template v-else-if="column.key === 'progress'">
                  <a-progress
                    v-if="record.target && record.last_approved_value != null"
                    :percent="
                      calculateProgress(
                        record.last_approved_value,
                        record.target
                      )
                    "
                    size="small"
                ...
                <template v-else-if="column.key === 'actions'">
                  <a-space>
                    <a-button
                      ...
                      :disabled="isSubmitDisabled(record.latest_update_status)"
                      :loading="
                        submittingUpdate &&
    ...
    <a-modal
      ...
      :title="`Submit Progress Update for ${currentSubmittingAssignment?.kpi_name}`"
...
const isSubmitDisabled = (status) => {
  return (
    status && !["Approved", "Rejected", null, undefined, ""].includes(status)
  );
};
const submitButtonText = (status) => {
  return isSubmitDisabled(status) ? "Pending..." : "Submit Update";
};
...
const openSubmitUpdateModal = (assignment) => {
  if (!assignment || !assignment.id) {
    notification.error({
      message: "Error",
      description: "Cannot identify assignment.",
    });
    return;
  }
...
const handleSubmitUpdate = async () => {
  ...
  try {
    await store.dispatch("kpiValue/submitKpiUpdate", {
      assignmentId,
      updateData: payload,
    });
