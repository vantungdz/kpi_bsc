    ...
    >
      <a-row :gutter="12">
        ...
        <a-col :span="12">
          ...
          >
            <a-select
              v-model:value="form.created_by_type"
      ...
      <a-form-item
        ...
        class="textLabel"
        label="Assign To Department/Section & Set Targets"
        name="section_id_table"
        help="Use this table to assign the KPI down and set targets."
        :extra="
          form.assigned_user_id ? 'Direct user assignment will be cleared.' : ''
        "
      >
        <a-alert
        ...
        >
          <template #target="{ record }">
            <a-input-number
              :value="targetValues[record.key] || null"
              placeholder="Target"
              :min="0"
              style="width: 100%"
              :disabled="
...
const loadKpiTemplate = async (selectedId) => {
  ...
  try {
    ...
    if (kpiDetail) {
      ...
      form.value.created_by_type = kpiDetail.created_by_type || null;
...
const handleChangeCreate = async () => {
  ...
  try {
    ...
    const kpiData = {
      ...
      created_by_type: form.value.created_by_type, // <<< Thêm created_by_type
