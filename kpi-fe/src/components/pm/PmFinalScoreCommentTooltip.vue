...
/** Nhận xét đánh giá — `evidences.gmComment` (PM ghi trước, GM ghi đè). */
...
const tooltipText = computed(() => String(props.comment ?? '').trim())
const hasContent = computed(() => tooltipText.value.length > 0)
...
<template>
  ...
  >
    <button
      ...
      aria-label="Evaluation comment"
      :tabindex="hasContent ? 0 : -1"
      :disabled="!hasContent"
      @click.stop
    >
      <span
        class="inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-slate-300 text-[10px] font-bold leading-none text-slate-700 cursor-pointer"
      >
        ?
      </span>
    </button>
