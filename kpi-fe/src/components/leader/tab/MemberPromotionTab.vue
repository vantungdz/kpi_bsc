          ...
          <template v-for="(category, catIndex) in sortedCategories" :key="`pp-${catIndex}`">
            ...
            <template v-for="(assign, assignIndex) in category.assignments" :key="assign.assignmentId">
              <tr class="group bg-white transition-colors hover:bg-slate-50">
                ...
                <td class="px-4 py-3 align-middle">
                  <p class="text-xs font-bold text-violet-700">{{ parseEvidence(assign.evidences).result || '-' }}</p>
                </td>
                ...
                <td class="px-4 py-3 text-center align-middle">
                  <button
                    type="button"
                    class="inline-flex min-w-28 items-center justify-between gap-1 rounded-lg border border-violet-200 bg-violet-50 px-2 py-1.5 text-[11px] font-bold text-violet-700"
                    @click="toggleRow(assign.assignmentId)"
                  >
