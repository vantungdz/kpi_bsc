...
// Tính tổng điểm weight từ API data
...
<template>
  <div class="flex flex-col h-full">
      ...
      <div v-for="(assign, assignIndex) in category.assignments" :key="assign.assignmentId"
        ...
        <div class="flex items-start justify-between gap-3 px-5 py-4">
          <div class="min-w-0 flex-1">
            <div class="flex items-start gap-2 cursor-pointer" @click="toggleMemberEvidence(assign.assignmentId)">
              ...
              <div>
                ...
                <p class="mt-1.5 max-w-md rounded border border-indigo-100 bg-indigo-50/90 px-2 py-1 text-[10px] font-medium leading-snug text-indigo-900 line-clamp-2">
                  <i class="fas fa-certificate mr-1 shrink-0 text-indigo-500"/>
                  Mock Certificate Outcome Note
                </p>
              </div>
            </div>
          </div>
          <div class="flex shrink-0 flex-wrap items-center justify-end gap-2">
            ...
            <button type="button"
                    class="inline-flex max-w-36 items-center justify-between gap-1 rounded-lg border px-2 py-1.5 text-[11px] font-bold shadow-sm transition-colors bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"
                    @click="toggleMemberEvidence(assign.assignmentId)">
                            <span class="flex min-w-0 items-center gap-1.5">
                                <i class="fas fa-file shrink-0 text-xs"/>
        ...
        <div v-show="isMemberEvidenceOpen(assign.assignmentId)"
             class="border-t px-5 py-3 border-indigo-100 bg-indigo-50/40">
          ...
          <div class="overflow-x-auto rounded-lg border border-indigo-200">
            <table class="w-full text-xs">
              <thead>
              <tr class="border-b border-indigo-200 bg-indigo-50 text-indigo-900">
    ...
    <div class="border-t border-slate-200 bg-white px-5 py-4 pb-8">
      ...
      <div class="space-y-3">
        <div>
          <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">Nhân viên</label>
          <div class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-400 italic">
            Chưa có nhận xét. (Mock)
          </div>
        </div>
        <div>
          <label class="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">Supervisor</label>
          <div class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-400 italic">
            Chưa có nhận xét. (Mock)
          </div>
        </div>
