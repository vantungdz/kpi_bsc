...
function onEvidenceSaved(payload: any) {
  ...
  if (selectedKpi.value) {
    ...
    selectedKpi.value.evidenceNote = payload.evidenceNote;
  }
...
const totals = computed(() => {
  ...
  // Tính điểm các KPI Promotion gốc từ API
...
async function submitEvaluation() {
  ...
  try {
    await pmKpiService.bulkUpdateKpiStatus({
      ...
      promotion: true // Flag cho backend
    });

    toast.success('Update KPI Promotion statuses successfully!');
    fetchData(); // Tải lại dữ liệu bảng
  } catch (error) {
    console.error('Failed to update KPI Promotion statuses', error);
    toast.error('Có lỗi xảy ra khi cập nhật trạng thái Promotion KPI.');
  }
}
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-sm">
          ...
          <tr v-if="!apiData?.categories?.length && !loading">
            <td colspan="9" class="p-8 text-center text-slate-500">Chưa có dữ liệu Promotion KPI cho năm {{ year }}</td>
          </tr>
