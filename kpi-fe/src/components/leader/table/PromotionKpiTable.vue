...
async function fetchData() {
  loading.value = true;
  try {
    // Chỉ khác type so với Personal KPI
    const data = await leaderKpiService.getKpiInfo(props.year, 'PROMOTION');
    apiData.value = data;
  } catch (error) {
...
function submitEvaluation() {
  ...
  console.log("Submit Đánh Giá lên PM (Promotion)", {
    year: props.year,
    totals: totals.value,
    scores: {...selfScores},
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-sm">
          ...
          <tr v-if="!apiData?.categories?.length && !loading">
            <td colspan="8" class="p-8 text-center text-slate-500">Chưa có dữ liệu Promotion KPI cho năm {{ year }}</td>
          </tr>
