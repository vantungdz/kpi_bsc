...
async function save() {
  ...
  try {
    ...
    const response = await gmKpiService.createStrategicKpi(requestBody)
          ...
          <div class="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6">
            ...
            <div class="ind-kpi-section-card rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div class="space-y-4">
                ...
                <div>
                  <label class="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Description
                    <span class="text-[9px] font-normal normal-case text-slate-400">(Optional)</span></label>
                  <textarea v-model="description" rows="2" placeholder="Giải thích ngắn gọn về cách đo lường..."
