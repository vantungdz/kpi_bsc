...
const layoutYear = inject<ComputedRef<number> | undefined>('gmEvaluationYear', undefined)
const employees = computed(() => props.employees)
const isPmList = computed(() => props.listEntity === 'pm')
const usePmTree = computed(
  () => isPmList.value && Array.isArray(props.pmBranches) && props.pmBranches.length > 0,
)
const codeColumnLabel = computed(() => (isPmList.value ? 'Mã PM' : 'Mã NV'))
const nameColumnLabel = computed(() => (isPmList.value ? 'Tên PM' : 'Tên Nhân viên'))
const gmScoreColumnLabel = computed(() => (isPmList.value ? 'Điểm GM' : 'Điểm PM'))
const gmScoreDetailLabel = computed(() => (isPmList.value ? 'GM Score' : 'PM Score'))
const searchPlaceholder = computed(() =>
  usePmTree.value
    ? 'Lọc theo section, PM, Leader, nhân viên hoặc mã...'
    : isPmList.value
      ? 'Lọc theo tên PM hoặc đơn vị (vd: Liêm, QA)...'
      : 'Lọc theo tên nhân viên (vd: Huy, Phước)...',
)
const emptyFilterMessage = computed(() =>
  isPmList.value ? 'Không có PM khớp bộ lọc.' : 'Không có nhân viên khớp bộ lọc.',
)
...
function flattenPmBranchSheetHolders(br: GmEvalPmBranch): GmEvalMember[] {
  return [br.pm, ...br.leaders.flatMap((l) => [l.sheet, ...l.members]), ...br.directMembers]
}
...
function pmBranchHasPending(br: GmEvalPmBranch): boolean {
  ...
  return [...br.leaders.flatMap((l) => l.members), ...br.directMembers].some((m) => m.status === 'pending_pm')
}
...
function roleTagClass(kind: 'pm' | 'leader') {
  const base =
  ...
  if (kind === 'pm') return `${base} border-indigo-200 bg-indigo-50 text-indigo-800`
  return `${base} border-amber-200 bg-amber-50 text-amber-900`
}

const totalCount = computed(() => employees.value.length)

const pendingCount = computed(() => employees.value.filter((e) => e.status === 'pending_pm').length)
...
const drawerActiveKpiGroups = computed((): GmKpiGroup[] =>
  drawerEvalTab.value === 'cascade' ? drawerCascadeGroups.value : drawerPromotionGroups.value,
)
...
function statusBadgeClass(emp: GmEvalMember) {
  const base =
    'inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold sm:gap-1.5 sm:px-2.5 sm:py-1 sm:text-xs'
...
watch([nameFilter, listFilter], () => {
  ...
  if (ex && !filteredEmployees.value.some((e) => e.id === ex)) drawerEmpId.value = null
})
...
function confirmDone(emp: GmEvalMember) {
  if (isReadonly.value) return
...
<template>
  <div class="w-full max-w-none pb-6 sm:pb-8">
    ...
    >
      ...
      <p>
        <span class="font-bold">Chế độ chỉ xem:</span> năm {{ effectiveYear }} đã khóa kỳ. Không thể chỉnh điểm PM hay ghi chú supervisor.
      </p>
    </div>

    <!-- Tiêu đề — padding trong khung giống header `p-5` của GmKpiDiagnosticsTable -->
    <div class="border-b border-slate-200 bg-white px-4 pb-4 pt-4 sm:px-5 sm:pb-5 sm:pt-5">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
          ...
          >
            <i class="fas fa-clipboard-check text-[11px] text-blue-600 sm:text-xs" aria-hidden="true" />
            KPI management &amp; evaluation (GM)
          </h3>
          <p v-if="contextSubtitle" class="mt-1 text-[10px] font-bold text-indigo-700 sm:text-xs">
            {{ contextSubtitle }}
          </p>
        </div>
        <div v-if="showYearDropdown" class="flex shrink-0 flex-wrap gap-2 self-start lg:self-auto">
        <select
    ...
    <div class="flex flex-col overflow-hidden bg-white">
      ...
      >
        ...
        <div class="flex shrink-0 gap-2 overflow-x-auto pb-0.5 sm:pb-0">
          <button
            ...
            :class="listFilter === 'all' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'"
          ...
          <button
            ...
            :class="listFilter === 'pending' ? 'border-rose-600 bg-rose-600 text-white' : 'border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100'"
            @click="listFilter = 'pending'"
          >
            <span class="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-80" />
      ...
      <div class="overflow-x-auto">
        ...
        >
          <thead>
            ...
            >
              ...
              <th class="min-w-[11rem] px-3 py-2.5 sm:min-w-[13rem] sm:px-4 sm:py-3">
                {{ nameColumnLabel }}
              </th>
              ...
              <th class="bg-slate-50/80 px-3 py-2.5 text-center sm:px-4 sm:py-3">
                Điểm Tự chấm
              </th>
              <th class="bg-indigo-50/40 px-3 py-2.5 text-center text-indigo-800 sm:px-4 sm:py-3">
                {{ gmScoreColumnLabel }}
              </th>
              <th class="w-[4.5rem] px-2 py-2.5 text-center sm:w-20 sm:px-3 sm:py-3">
                Thao tác
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-200 text-xs sm:text-sm">
            <template v-for="emp in filteredEmployees" :key="emp.id">
              <tr
                ...
                :class="drawerEmpId === emp.id ? 'bg-slate-100/80 border-l-4 border-indigo-500' : ''"
              >
                <td class="px-3 py-2.5 font-bold text-slate-400 sm:px-4 sm:py-3">
                  {{ emp.code }}
                </td>
                ...
                <td class="px-2 py-2.5 text-center sm:px-3" @click.stop>
                  <button
                    v-if="canOpenEvaluationDrawer(emp)"
                    type="button"
                    class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-indigo-200 bg-white text-indigo-600 shadow-sm transition-colors hover:bg-indigo-50 hover:text-indigo-800"
                    title="Mở đánh giá KPI"
        ...
        >
          ...
          <tbody class="divide-y divide-slate-200 text-xs sm:text-sm">
            <template v-for="sec in pmEvalSectionsForDisplay" :key="sec.sectionId">
              <tr
                ...
                <td colspan="6" class="px-4 py-2 sm:px-5">
                  <div class="flex flex-wrap items-center gap-2">
                    <i
                      ...
                      :class="expandedSectionIds[sec.sectionId] ? 'rotate-90' : ''"
                      aria-hidden="true"
                    />
                    <i class="fas fa-sitemap text-[10px] text-slate-500 sm:text-xs" aria-hidden="true" />
                    <span class="text-[10px] font-extrabold uppercase tracking-wide text-slate-700 sm:text-[11px]">
                      {{ sec.sectionName }}
                    </span>
                    <span class="text-[10px] font-semibold normal-case text-slate-500 sm:text-xs">
              ...
              <tr class="border-0 bg-transparent">
                <td colspan="6" class="border-y-0 p-0 align-top">
                  <div
                    ...
                    :class="expandedSectionIds[sec.sectionId] ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'"
                  >
                    <div class="min-h-0">
                      <table class="gm-eval-hub-table w-full min-w-[800px] table-fixed text-left whitespace-nowrap text-xs sm:text-sm">
                        <colgroup>
                          <col style="width: 36%" />
                          <col style="width: 8%" />
                          <col style="width: 18%" />
                        ...
                        <tbody class="divide-y divide-slate-200">
                          <template v-for="br in [sec.branch]" :key="br.pm.id">
                <tr
                  class="hover:bg-slate-50/80 transition-colors group"
                  :class="[
                    pmBranchHasChildren(br) ? 'cursor-pointer' : '',
                    drawerEmpId === br.pm.id ? 'bg-slate-100/80 border-l-4 border-indigo-500' : '',
                  ]"
                  @click="pmBranchHasChildren(br) && togglePmBranchExpand(br.pm.id)"
                >
                <td class="px-3 py-2.5 sm:px-4 sm:py-3">
                  <div class="flex items-center gap-1.5 sm:gap-2">
                    <button
                      ...
                      @click.stop="togglePmBranchExpand(br.pm.id)"
                    >
                      <i
                        class="fas fa-chevron-right text-[10px] transition-transform duration-300 ease-out motion-reduce:transition-none sm:text-xs"
                        :class="expandedPmIds[br.pm.id] ? 'rotate-90' : ''"
                      />
                    </button>
                    <span v-else class="inline-block w-7 shrink-0 sm:w-8" />
                    <div
                      class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold sm:h-8 sm:w-8 sm:text-xs"
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      >
                        ...
                        <span class="font-semibold text-slate-400">({{ br.pm.code }})</span>
                        <span :class="roleTagClass('pm')">PM</span>
                      </p>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-3 py-2.5 text-center sm:px-4 sm:py-3">
                  <span :class="statusBadgeClass(br.pm)">
                    ...
                    />
                    <i
                      v-else-if="br.pm.status === 'self_scoring'"
                      class="fas fa-pen text-[10px]"
                    />
                    <i
                      v-else
                      class="fas fa-check text-[10px]"
                    />
                    {{ statusLabelGm(br.pm.status) }}
                  </span>
                </td>
                <td class="bg-slate-50/80 px-3 py-2.5 text-center sm:px-4 sm:py-3">
                  <span v-if="br.pm.selfScoreDisplay" class="font-bold text-slate-700">{{ br.pm.selfScoreDisplay }}</span>
                  <span v-else class="font-medium italic text-slate-400">-</span>
                </td>
                <td class="bg-indigo-50/40 px-3 py-2.5 text-center sm:px-4 sm:py-3">
                  <span :class="pmPreviewClass(br.pm)">{{ pmPreviewText(br.pm) }}</span>
                </td>
                <td class="px-2 py-2.5 text-center sm:px-3" @click.stop>
                  <button
                    v-if="canOpenEvaluationDrawer(br.pm)"
                    type="button"
                    class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-indigo-200 bg-white text-indigo-600 shadow-sm transition-colors hover:bg-indigo-50 hover:text-indigo-800"
                    title="Mở đánh giá KPI"
                    aria-label="Mở đánh giá KPI"
                          ...
                          <template v-if="pmBranchHasChildren(br)">
                            <tr class="border-0 bg-transparent">
                              <td colspan="6" class="border-y-0 p-0 align-top">
                                <div
                                  class="grid overflow-hidden transition-[grid-template-rows] duration-300 ease-in-out motion-reduce:transition-none"
                                  :class="expandedPmIds[br.pm.id] ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'"
                                >
                                  <div class="min-h-0">
                                    <table class="gm-eval-hub-table w-full min-w-[800px] table-fixed text-left whitespace-nowrap text-xs sm:text-sm">
                                      <colgroup>
                                      ...
                                      <tbody class="divide-y divide-slate-200">
                                        <template v-for="ld in br.leaders" :key="ld.leaderKey">
                  <tr
                    class="bg-slate-50/50 transition-colors group"
                    :class="[
                      ld.members.length ? 'cursor-pointer hover:bg-slate-50/90' : '',
                      drawerEmpId === ld.sheet.id ? 'bg-slate-100/80 border-l-4 border-indigo-500' : '',
                    ]"
                    @click="ld.members.length && toggleLeaderBranchExpand(ld.leaderKey)"
                  >
                    <td class="px-3 py-2 sm:px-4 sm:py-2.5">
                      <div class="flex items-center gap-1.5 pl-1 sm:pl-2">
                        <button
                          ...
                          @click.stop="toggleLeaderBranchExpand(ld.leaderKey)"
                        >
                          <i
                            class="fas fa-chevron-right text-[10px] transition-transform duration-300 ease-out motion-reduce:transition-none sm:text-xs"
                            :class="expandedLeaderKeys[ld.leaderKey] ? 'rotate-90' : ''"
                          />
                        </button>
                        <span v-else class="ml-5 inline-block w-7 shrink-0 sm:ml-6 sm:w-8" />
                        <div
                          class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold sm:h-8 sm:w-8 sm:text-xs"
                          :class="ld.sheet.initialsClass"
                        >
                          {{ ld.sheet.initials }}
                        </div>
                        <div class="min-w-0 flex-1">
                          ...
                          >
                            <span>{{ ld.sheet.name }}</span>
                            <span class="font-semibold text-slate-400">({{ ld.sheet.code }})</span>
                            <span :class="roleTagClass('leader')">Leader</span>
                          </p>
                        </div>
                      </div>
                    </td>
                    <td class="px-3 py-2 text-center sm:px-4 sm:py-2.5">
                      <span
                        class="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-600 sm:px-2 sm:py-1 sm:text-xs"
                      >
                        {{ ld.sheet.rank }}
                    ...
                    <td class="px-3 py-2 text-center sm:px-4 sm:py-2.5">
                      <span :class="statusBadgeClass(ld.sheet)">
                        <span
                          v-if="ld.sheet.status === 'pending_pm'"
                          class="h-1.5 w-1.5 shrink-0 rounded-full bg-rose-500 animate-pulse"
                        />
                        <i
                          v-else-if="ld.sheet.status === 'self_scoring'"
                          class="fas fa-pen text-[10px]"
                        />
                        <i
                          v-else
                          class="fas fa-check text-[10px]"
                        />
                        {{ statusLabelGm(ld.sheet.status) }}
                      </span>
                    </td>
                    <td class="bg-slate-50/80 px-3 py-2 text-center sm:px-4 sm:py-2.5">
                      <span v-if="ld.sheet.selfScoreDisplay" class="font-bold text-slate-700">{{ ld.sheet.selfScoreDisplay }}</span>
                      <span v-else class="font-medium italic text-slate-400">-</span>
                    </td>
                    <td class="bg-indigo-50/40 px-3 py-2 text-center sm:px-4 sm:py-2.5">
                      <span :class="pmPreviewClass(ld.sheet)">{{ pmPreviewText(ld.sheet) }}</span>
                    </td>
                    <td class="px-2 py-2 text-center sm:px-3 sm:py-2.5" @click.stop>
                      <button
                        v-if="canOpenEvaluationDrawer(ld.sheet)"
                        type="button"
                        class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-indigo-200 bg-white text-indigo-600 shadow-sm transition-colors hover:bg-indigo-50 hover:text-indigo-800"
                        title="Mở đánh giá KPI"
                        aria-label="Mở đánh giá KPI"
                        @click="toggleEvaluationDrawer(ld.sheet)"
                      >
                        <i class="fas fa-clipboard-check text-xs" aria-hidden="true" />
                      </button>
                      <span v-else class="text-[11px] font-medium text-slate-300">—</span>
                                        ...
                                        <template v-if="ld.members.length">
                                          <tr class="border-0 bg-transparent">
                                            <td colspan="6" class="border-y-0 p-0 align-top">
                                              <div
                                                class="grid overflow-hidden transition-[grid-template-rows] duration-300 ease-in-out motion-reduce:transition-none"
                                                :class="expandedLeaderKeys[ld.leaderKey] ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'"
                                              >
                                                <div class="min-h-0">
                                                  <table class="gm-eval-hub-table w-full min-w-[800px] table-fixed text-left whitespace-nowrap text-xs sm:text-sm">
                                                    <colgroup>
                        ...
                        <div class="flex items-center gap-1.5 pl-1 sm:pl-2">
                          ...
                          >
                            ...
                            <div class="min-w-0 flex-1">
                              ...
                              >
                                <span>{{ emp.name }}</span>
                                <span class="font-semibold text-slate-400">({{ emp.code }})</span>
                              </p>
                            </div>
                          </div>
                        </div>
      ...
      <div
        v-if="usePmTree ? displayedPmBranches.length === 0 : filteredEmployees.length === 0"
              ...
              <div class="min-w-0">
                ...
                <h2 class="truncate text-base font-bold text-slate-900 sm:text-lg">
                  <template v-if="isPmList">
                    {{ drawerEmployee.name }}
                    <span class="font-semibold text-slate-500">({{ drawerEmployee.code }})</span>
                  </template>
                  <template v-else>
                    {{ drawerEmployee.name }}
                  </template>
                ...
                <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md">
                  <div class="border-b border-slate-200 bg-slate-50/80 px-4 py-4 sm:px-5">
                    <h3 class="flex items-center gap-2 text-base font-bold text-slate-800">
                      <i class="fas fa-list text-indigo-600" aria-hidden="true" />
                      Bảng đánh giá chi tiết — {{ effectiveYear }}
                    </h3>
                    <p class="mt-0.5 text-[11px] font-medium text-slate-500 sm:text-xs">
                      {{ flattenGmKpiItems(drawerEmployee).length }} hạng mục tổng cộng
                  ...
                  <div class="border-b border-slate-200 bg-white">
                    ...
                    >
                      ...
                      >
                        ...
                        <span class="block font-semibold opacity-90 sm:inline sm:ml-0.5">({{ drawerCascadeItemCount }})</span>
                      </button>
                      <button
                        type="button"
                        role="tab"
                      ...
                      >
                        Promotion
                        <span class="block font-semibold opacity-90 sm:inline sm:ml-0.5">({{ drawerPromotionItemCount }})</span>
                      </button>
                    </div>
                    ...
                    <table class="w-full text-left">
                      <thead>
                        ...
                        >
                          ...
                          <th class="min-w-[18rem] px-4 py-3 sm:min-w-[22rem] lg:min-w-[26rem]">Hạng mục (Objectives)</th>
                          <th class="w-28 min-w-[7rem] px-4 py-3 text-center sm:w-32">Trọng số</th>
                          <th class="w-44 min-w-[11rem] px-4 py-3 text-center sm:w-52 sm:min-w-[13rem]">
                            Bằng chứng
                          </th>
                          <th class="w-32 min-w-[8rem] px-4 py-3 text-center sm:w-36">Self Score</th>
                          <th
                          ...
                          >
                            {{ gmScoreDetailLabel }} <span class="text-rose-500">*</span>
                          </th>
                        </tr>
                      </thead>
                      <tbody v-if="drawerActiveKpiGroups.length === 0" class="text-sm">
                        <tr>
                          <td colspan="6" class="px-4 py-12 text-center text-sm font-medium text-slate-500">
                            Không có KPI trong tab này.
                          </td>
                        </tr>
                      </tbody>
                      <tbody v-else class="divide-y divide-slate-200 text-sm">
                        <template v-for="group in drawerActiveKpiGroups" :key="group.groupTitle">
                          ...
                          <template v-for="item in group.items" :key="item.id">
                            <tr class="transition-colors hover:bg-slate-50/50">
                              ...
                              <td class="px-4 py-4">
                                ...
                                <p class="mt-0.5 text-[11px] text-slate-500">{{ item.target }}</p>
                              </td>
                              <td class="min-w-[7rem] px-4 py-4 text-center sm:min-w-[8rem]">
                                <span class="rounded bg-slate-100 px-2 py-1 font-bold text-slate-700">{{ item.weight }}</span>
                              </td>
                              <td class="min-w-[11rem] px-4 py-4 text-center sm:min-w-[13rem]">
                                <button
                                  type="button"
                                  ...
                                  :class="[
                                    ...
                                    evidenceOpen(drawerEmployee.id, item.id) ? 'ring-2 ring-blue-300' : '',
                                  ]"
                                  @click.stop="toggleEvidence(drawerEmployee.id, item.id)"
                                >
                                  ...
                                  <i
                                    ...
                                    :class="evidenceOpen(drawerEmployee.id, item.id) ? 'rotate-180' : ''"
                                  />
                                </button>
                              </td>
                              <td class="min-w-[8rem] px-4 py-4 text-center sm:min-w-[9rem]">
                                <div
                              ...
                              <td class="min-w-[9rem] border-l border-indigo-50 bg-indigo-50/30 px-4 py-4 text-center sm:min-w-[10rem]">
                                <select
                                  ...
                                  :class="pmSelectClass(drawerEmployee, item)"
                                  :disabled="isReadonly"
                                  :value="pmScores[drawerEmployee.id]?.[item.id] ?? ''"
                                  @change="
                                    setPmScore(
                                      drawerEmployee.id,
                                      item.id,
                                ...
                                >
                                  <option value="">-</option>
                                  <option v-for="n in 5" :key="n" :value="6 - n">{{ 6 - n }}</option>
                                </select>
                              </td>
                            </tr>
                            <tr v-show="evidenceOpen(drawerEmployee.id, item.id)" class="bg-slate-100/40">
                              <td colspan="6" class="p-0" :class="evidencePanelBorder(item.evidence.accent)">
                                <div class="border-l-4 p-4 md:p-6" :class="evidenceAccentBorder(item.evidence.accent)">
                                  <h4 class="mb-3 flex items-center text-sm font-bold text-slate-800">
                                    <i :class="[item.evidence.icon, 'mr-2 w-4 text-indigo-600']" />
                                    {{ item.evidence.title }}
                                  </h4>
                                  <p v-if="item.evidence.attachmentUrl" class="mb-3">
                                    <a
                                    ...
                                    >
                                      <i class="fas fa-paperclip shrink-0 text-xs" />
                                      <span>{{
                                        item.evidence.attachmentLabel ?? 'Xem bằng chứng đính kèm'
                                      }}</span>
                                    </a>
                                  </p>
                                  <table
                      ...
                      <tfoot class="border-t-2 border-slate-200 bg-slate-100/80 text-xs font-semibold text-slate-700">
                        <tr>
                          <td
                            colspan="2"
                          ...
                          <td class="px-4 py-2.5 text-center text-slate-800">
                            {{ totalKpiWeightForGroupList(drawerActiveKpiGroups) }}
                          </td>
                          <td colspan="3" />
                        </tr>
                        <tr class="border-t border-slate-200 bg-white">
                          ...
                          <td class="border-x border-slate-200 px-4 py-2.5 text-center">
                            <span class="text-base font-bold tabular-nums text-slate-800 sm:text-lg">{{ selfAvgForGroupList(drawerEmployee, drawerActiveKpiGroups) }}</span>
                          </td>
                          <td />
                        </tr>
                        <tr class="border-t border-slate-200 bg-indigo-50/90">
                          <td
                            colspan="5"
                          ...
                          <td class="bg-indigo-100/90 px-4 py-2.5 text-center shadow-inner">
                            <span class="text-base font-bold tabular-nums text-indigo-800 sm:text-lg">{{ pmAvgForGroupList(drawerEmployee, drawerActiveKpiGroups) }}</span>
                          </td>
                        </tr>
                      </tfoot>
                    </table>
                  ...
                  <div class="border-t border-slate-200 bg-white p-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
                      ...
                      <div class="space-y-2">
                        <label class="block text-[11px] font-bold uppercase tracking-wider text-indigo-600">
                    ...
                    <div class="mt-5 flex justify-end gap-2 border-t border-slate-100 pt-3 sm:mt-6 sm:gap-2.5 sm:pt-4">
                      <button
                        type="button"
                      ...
                      >
                        <i class="fas fa-check-circle text-[11px] sm:text-xs" aria-hidden="true" />
                        Xác nhận hoàn tất
                      </button>
                    </div>
                  </div>
