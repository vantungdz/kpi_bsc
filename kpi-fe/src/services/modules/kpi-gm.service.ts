...
export async function apiListGmDepartmentMemberCandidates(
  ...
  return http
    .get<ApiResponse<GmDepartmentMemberCandidateApiRow[]>>(
      `/kpi/gm/departments/${id}/member-candidates`,
      { params },
    )
    .then((r) => r.data.data)
}
...
export async function apiAddGmDepartmentMembers(
  ...
  const id = encodeURIComponent(departmentId.trim())
  return http
    .post<ApiResponse<GmDepartmentApiRow>>(`/kpi/gm/departments/${id}/members`, body)
    .then((r) => r.data.data)
}

/** DELETE /kpi/gm/departments/:id/members/:userId */
export async function apiRemoveGmDepartmentMember(departmentId: string, userId: string): Promise<void> {
  const did = encodeURIComponent(departmentId.trim())
  const uid = encodeURIComponent(userId.trim())
  await http.delete<ApiResponse<null>>(`/kpi/gm/departments/${did}/members/${uid}`)
}

/** GET /kpi/gm/departments — danh sách phòng ban; `year` lọc KPI team theo năm chu kỳ. */
export async function apiListGmDepartments(year?: number): Promise<GmDepartmentApiRow[]> {
  return http
    .get<ApiResponse<GmDepartmentApiRow[]>>('/kpi/gm/departments', { params: year != null ? { year } : {} })
    .then((r) => r.data.data)
}

/** POST /kpi/gm/departments — tạo phòng ban (không có mã code). */
export async function apiCreateGmDepartment(body: GmCreateDepartmentBody): Promise<GmDepartmentApiRow> {
  return http
...
export async function apiUpdateGmDepartment(
  ...
  const id = encodeURIComponent(departmentId.trim())
  return http.put<ApiResponse<GmDepartmentApiRow>>(`/kpi/gm/departments/${id}`, body).then((r) => r.data.data)
}

/** DELETE /kpi/gm/departments/:id — xóa mềm. */
export async function apiDeleteGmDepartment(departmentId: string): Promise<ApiResponse<null>> {
  const id = encodeURIComponent(departmentId.trim())
  return http.delete<ApiResponse<null>>(`/kpi/gm/departments/${id}`).then((r) => r.data)
}

/** Tránh gọi trùng `diagnostics-hierarchy` cùng `year` khi layout + drawer chạy song song. */
const diagnosticsHierarchyInflight = new Map<number, Promise<GmDiagnosticsHierarchyApiData>>()

/** GET /kpi/gm/diagnostics-hierarchy?year= — catalog + Strategic diagnostics tree */
export async function apiGetGmDiagnosticsHierarchy(year: number): Promise<GmDiagnosticsHierarchyApiData> {
  const cached = diagnosticsHierarchyInflight.get(year)
  if (cached) return cached

  const p = http
    .get<ApiResponse<GmDiagnosticsHierarchyApiData>>('/kpi/gm/diagnostics-hierarchy', { params: { year } })
    .then((r) => r.data.data)
    .finally(() => {
...
export async function apiGetGmKpiCategories(): Promise<GmKpiCategoryItem[]> {
  return http.get<ApiResponse<GmKpiCategoryItem[]>>('/kpi/gm/kpi-categories').then((r) => r.data.data)
}
...
export async function apiGetGmKpiCyclesWithKpis(): Promise<GmKpiCycleOption[]> {
  return http.get<ApiResponse<GmKpiCycleOption[]>>('/kpi/gm/kpi-cycles-with-kpis').then((r) => r.data.data)
}

/** GET /kpi/gm/kpi-cycles-for-evaluation — `year` ≥ năm hiện tại (dropdown năm đánh giá / header). */
export async function apiGetGmKpiCyclesForEvaluation(): Promise<GmKpiCycleOption[]> {
  return http.get<ApiResponse<GmKpiCycleOption[]>>('/kpi/gm/kpi-cycles-for-evaluation').then((r) => r.data.data)
}
...
export async function apiGetGmEvaluationHubAssignments(
  ...
  return http
    .get<ApiResponse<GmEvaluationHubApiResponse>>('/kpi/gm/evaluation-hub/assignments', {
      params: { cycleId },
    })
    .then((r) => r.data.data)
}
...
export async function apiPostGmEvaluationHubConfirm(
  ...
  return http
    .post<ApiResponse<GmEvaluationHubConfirmResult>>('/kpi/gm/evaluation-hub/confirm', body)
    .then((r) => r.data.data)
}

/** GET /kpi/gm/approved-kpi-queue?cycleId= — ASM 401/402/403 (tab Approved KPI). */
export async function apiGetGmApprovedKpiQueue(cycleId: string): Promise<GmApprovedKpiQueueItemApi[]> {
  return http
    .get<ApiResponse<GmApprovedKpiQueueItemApi[]>>(`/kpi/gm/approved-kpi-queue`, { params: { cycleId: cycleId.trim() } })
    .then((r) => r.data.data)
}
...
export async function apiPostGmApprovedKpiDecision(
  ...
  return http
    .post<ApiResponse<GmApprovedKpiDecisionResultApi>>('/kpi/gm/approved-kpi-queue/decision', body)
    .then((r) => r.data.data)
}
...
export async function apiCreateGmStrategicKpi(
  ...
  return http.post<ApiResponse<GmCreateStrategicKpiResponseData>>('/kpi/strategic-kpis', body).then((r) => r.data)
}

/** DELETE /kpi/strategic-kpis/:kpiInformationId — GM xóa KPI kỳ + assignments (mềm). */
export async function apiDeleteGmStrategicKpi(kpiInformationId: string): Promise<ApiResponse<null>> {
  const id = encodeURIComponent(kpiInformationId.trim())
  return http.delete<ApiResponse<null>>(`/kpi/strategic-kpis/${id}`).then((r) => r.data)
}

/** GET /kpi/strategic-kpis/:id — dữ liệu form sửa KPI. */
export async function apiGetGmStrategicKpiForEdit(kpiInformationId: string): Promise<GmStrategicKpiEditData> {
  const id = encodeURIComponent(kpiInformationId.trim())
  return http.get<ApiResponse<GmStrategicKpiEditData>>(`/kpi/strategic-kpis/${id}`).then((r) => {
    const d = r.data.data
...
export async function apiUpdateGmStrategicKpi(
  ...
  const id = encodeURIComponent(kpiInformationId.trim())
  return http.put<ApiResponse<GmCreateStrategicKpiResponseData>>(`/kpi/strategic-kpis/${id}`, body).then((r) => r.data)
}

/** GET /kpi/gm/kpi-templates — {@code kpi_templates}. */
export async function apiGetGmKpiTemplates(): Promise<GmKpiTemplatePackageRow[]> {
  return http.get<ApiResponse<GmKpiTemplatePackageRow[]>>('/kpi/gm/kpi-templates').then((r) => r.data.data)
}

/** GET /kpi/gm/kpi-templates/:id/items — mục trong gói + master. */
export async function apiGetGmKpiTemplateItems(templateId: string): Promise<GmKpiTemplateItemRow[]> {
  const id = encodeURIComponent(templateId.trim())
  return http
    .get<ApiResponse<GmKpiTemplateItemRow[]>>(`/kpi/gm/kpi-templates/${id}/items`)
    .then((r) => r.data.data)
}

/** POST /kpi/gm/kpi-templates — tạo gói mẫu. */
export async function apiCreateGmKpiTemplate(body: GmCreateKpiTemplateBody): Promise<GmKpiTemplatePackageRow> {
  return http
...
export async function apiUpdateGmKpiTemplate(
  ...
  const id = encodeURIComponent(templateId.trim())
  return http
    .put<ApiResponse<GmKpiTemplatePackageRow>>(`/kpi/gm/kpi-templates/${id}`, body)
    .then((r) => r.data.data)
}

/** DELETE /kpi/gm/kpi-templates/:templateId */
export async function apiDeleteGmKpiTemplate(templateId: string): Promise<ApiResponse<null>> {
  const id = encodeURIComponent(templateId.trim())
  return http.delete<ApiResponse<null>>(`/kpi/gm/kpi-templates/${id}`).then((r) => r.data)
}
...
export async function apiCreateGmKpiTemplateItem(
  ...
  const id = encodeURIComponent(templateId.trim())
  return http
    .post<ApiResponse<GmKpiTemplateItemRow>>(`/kpi/gm/kpi-templates/${id}/items`, body)
    .then((r) => r.data.data)
}
...
export async function apiUpdateGmKpiTemplateItem(
  ...
  return http
    .put<ApiResponse<GmKpiTemplateItemRow>>(`/kpi/gm/kpi-templates/${tid}/items/${iid}`, body)
    .then((r) => r.data.data)
}

/** DELETE /kpi/gm/kpi-templates/:templateId/items/:itemId */
export async function apiDeleteGmKpiTemplateItem(templateId: string, itemId: string): Promise<ApiResponse<null>> {
  const tid = encodeURIComponent(templateId.trim())
  const iid = encodeURIComponent(itemId.trim())
  return http.delete<ApiResponse<null>>(`/kpi/gm/kpi-templates/${tid}/items/${iid}`).then((r) => r.data)
}
...
export const gmKpiService = {
  ...
  listDepartmentMemberCandidates: (departmentId: string, opts?: { q?: string; rankCodes?: string[] }) =>
    apiListGmDepartmentMemberCandidates(departmentId, opts),
  addDepartmentMembers: (departmentId: string, body: GmAddDepartmentMembersBody) =>
    apiAddGmDepartmentMembers(departmentId, body),
  removeDepartmentMember: (departmentId: string, userId: string) => apiRemoveGmDepartmentMember(departmentId, userId),
  createDepartment: (body: GmCreateDepartmentBody) => apiCreateGmDepartment(body),
  updateDepartment: (departmentId: string, body: GmUpdateDepartmentBody) => apiUpdateGmDepartment(departmentId, body),
  deleteDepartment: (departmentId: string) => apiDeleteGmDepartment(departmentId),
  ...
  createStrategicKpi: (body: Record<string, unknown>) => apiCreateGmStrategicKpi(body),
  deleteStrategicKpi: (kpiInformationId: string) => apiDeleteGmStrategicKpi(kpiInformationId),
  getStrategicKpiForEdit: (kpiInformationId: string) => apiGetGmStrategicKpiForEdit(kpiInformationId),
  updateStrategicKpi: (kpiInformationId: string, body: Record<string, unknown>) =>
    apiUpdateGmStrategicKpi(kpiInformationId, body),
  getKpiTemplates: () => apiGetGmKpiTemplates(),
  getKpiTemplateItems: (templateId: string) => apiGetGmKpiTemplateItems(templateId),
  createKpiTemplate: (body: GmCreateKpiTemplateBody) => apiCreateGmKpiTemplate(body),
  ...
  createKpiTemplateItem: (templateId: string, body: GmCreateKpiTemplateItemBody) =>
    apiCreateGmKpiTemplateItem(templateId, body),
  updateKpiTemplateItem: (templateId: string, itemId: string, body: GmUpdateKpiTemplateItemBody) =>
    apiUpdateGmKpiTemplateItem(templateId, itemId, body),
  deleteKpiTemplateItem: (templateId: string, itemId: string) => apiDeleteGmKpiTemplateItem(templateId, itemId),
  getEvaluationHubAssignments: (cycleId: string) => apiGetGmEvaluationHubAssignments(cycleId),
  confirmEvaluationHub: (body: GmEvaluationHubConfirmBody) => apiPostGmEvaluationHubConfirm(body),
  getApprovedKpiQueue: (cycleId: string) => apiGetGmApprovedKpiQueue(cycleId),
  decideApprovedKpiQueue: (body: GmApprovedKpiDecisionBody) => apiPostGmApprovedKpiDecision(body),
}
