...
const navItems = [
  { name: "KPI Overview", icon: "fas fa-chart-line", to: "/gm/dashboard" },
  {
    ...
    icon: "fas fa-sitemap",
    to: "/gm/settings/organization",
  },
  {
    name: "KPI Template",
    icon: "fas fa-layer-group",
    to: "/gm/settings/kpi-template",
  },
];
...
const gmKpiTemplateLibrary = reactive<{ openCreate: () => void }>({
  openCreate: () => {},
});
...
const isGmEvaluationRoute = computed(
  () => route.name === "gm-employee-evaluation",
);
const isGmCreateDepartmentRoute = computed(
  () => route.name === "gm-organization",
);
const isGmKpiTemplateRoute = computed(() => route.name === "gm-kpi-template");
const isGmRatingScaleRoute = computed(() => route.name === "gm-rating-scale");
const isGmReportsRoute = computed(() => route.name === "gm-reports");
const isGmSettingsRoute = computed(
  () =>
    isGmCreateDepartmentRoute.value ||
    isGmKpiTemplateRoute.value ||
    isGmRatingScaleRoute.value ||
    isGmReportsRoute.value,
);
...
const headerConfig = computed(() => {
  ...
  if (route.name === "gm-rating-scale") {
    return { category: "GM Workspace", title: "Khung điểm đánh giá" };
  }
...
/** Chu kỳ từ `GET /common/kpi-cycles` — mọi dòng `kpi_cycles` (chưa xóa mềm). */
...
type GmCycleContext = { cycleId: string; year: number };

/** Chu kỳ hợp lệ cho API — không fallback calendar year khi id rỗng / chưa có trong dropdown. */
function getCycleContext(): GmCycleContext | null {
  const cycleId = String(selectedCycleId.value ?? "").trim();
  if (!cycleId) return null;
  const row = gmHeaderCycleRows.value.find((c) => c.id === cycleId);
  if (row == null) return null;
  return { cycleId, year: row.year };
}

function isCurrentCycleContext(expected: GmCycleContext): boolean {
  const cur = getCycleContext();
  return cur != null && cur.cycleId === expected.cycleId;
}

/** Đồng bộ `GmKpiEvaluationPanel` / template — năm dương lịch suy ra từ chu kỳ đang chọn. */
const gmEvaluationYear = computed(() =>
  cycleYearFromCycleId(selectedCycleId.value),
);
/** Năm chu kỳ đã khóa — chỉ xem diagnostics / timeline, không sửa KPI chiến lược. */
const gmCycleReadonly = computed(() =>
  isReadonlyKpiYear(gmEvaluationYear.value),
);
...
const gmPersonalKpiLeaderIndividual = ref<LeaderKpiInformationResponse | null>(
  null,
);
const gmPersonalKpiLeaderPromotion = ref<LeaderKpiInformationResponse | null>(
  null,
);
...
watch(
  [selectedCycleId, gmHeaderCycleRows],
  async () => {
    ...
    const expected = ctx;
    try {
      const res = await kpiCycleService.getKpiCycleByYear(expected.year);
      if (!isCurrentCycleContext(expected)) return;
      gmDiagnosticsCycle.value = res ?? null;
    } catch {
      if (!isCurrentCycleContext(expected)) return;
      gmDiagnosticsCycle.value = null;
    }
...
const gmPersonalKpiAssignmentsById = computed(
  (): Record<string, LeaderKpiAssignment> => {
...
async function loadGmPersonalKpiRows() {
  ...
  try {
    const settled = await Promise.allSettled([
      leaderKpiService.getKpiInfo(expected.year, "INDIVIDUAL"),
      leaderKpiService.getKpiInfo(expected.year, "PROMOTION"),
    ]);
    if (!isCurrentCycleContext(expected)) return;

    const parts: Parameters<
      typeof mergeLeaderKpiInfoResponsesToGmPersonalRows
    >[0] = [];
    const errs: string[] = [];
    let leaderIndividual: LeaderKpiInformationResponse | null = null;
    let leaderPromotion: LeaderKpiInformationResponse | null = null;
    for (let i = 0; i < settled.length; i++) {
      const s = settled[i]!;
      const label = i === 0 ? "INDIVIDUAL" : "PROMOTION";
      if (s.status === "fulfilled") {
        if (label === "INDIVIDUAL") leaderIndividual = s.value;
        else leaderPromotion = s.value;
        parts.push({
          requestedType: label,
          response: s.value,
        });
    ...
    gmPersonalKpiLeaderIndividual.value = leaderIndividual;
    gmPersonalKpiLeaderPromotion.value = leaderPromotion;
    gmPersonalKpiRows.value =
      mergeLeaderKpiInfoResponsesToGmPersonalRows(parts);
    if (errs.length === 2) {
      pushGmNotification(errs.join(" · "), {
        variant: "error",
        durationMs: 8000,
      });
      gmPersonalKpiRows.value = [];
    ...
    gmPersonalKpiLeaderPromotion.value = null;
    pushGmNotification(
      e instanceof Error ? e.message : "Could not load personal KPIs",
      {
        variant: "error",
        durationMs: 8000,
      },
    ...
    if (isCurrentCycleContext(expected)) {
      gmPersonalKpiLoading.value = false;
    }
  }
}
...
const activeCycleLabel = computed(() => {
  const row = gmHeaderCycleRows.value.find(
    (c) => c.id === selectedCycleId.value,
  );
  if (row) return String(row.year);
  return (
    gmSelectableCycleOptions.value.find((c) => c.id === selectedCycleId.value)
      ?.label ?? ""
  );
});
...
/** Dropdown «Năm» ở header — tải ngay khi vào GM; không phụ thuộc form tạo KPI. */
async function loadGmHeaderEvaluationCycles() {
  try {
    const rows = await gmKpiService.getKpiCyclesForHeader();
...
async function loadProcessTimeline() {
  ...
  const expected = ctx;
  try {
    const data = await gmKpiService.getProcessTimeline(expected.cycleId);
    if (!isCurrentCycleContext(expected)) return;
    processTimelineData.value = data;
  } catch {
    if (!isCurrentCycleContext(expected)) return;
...
const pendingGmFeedbackEdit = ref<{
  assignmentId: string;
  kpiName: string;
} | null>(null);
...
const departments = ref(
  structuredClone(EMPTY_GM_WORKSPACE_CYCLE_SNAPSHOT.departments),
);
const mockMembersDetails = ref(
  structuredClone(EMPTY_GM_WORKSPACE_CYCLE_SNAPSHOT.membersDetails),
);
...
async function loadGmWorkspaceDepartmentsFromApi() {
  ...
  const expected = ctx;
  try {
    const rows = await gmKpiService.listDepartments(expected.year);
    if (!isCurrentCycleContext(expected)) return;
    departments.value = rows.map(mapGmDepartmentApiRowToWorkspaceMock);
    mockMembersDetails.value = departments.value.flatMap(
      (d) => d.staffDetails ?? [],
    );
  } catch {
    if (!isCurrentCycleContext(expected)) return;
...
const inactivePendingRowsForSelectedCycle = computed(() => {
  if (!useMockHub) {
    return approvedKpiQueueApiRows.value.filter(
      (row) =>
        Number(row.assignmentStatusCode) !== KPI_STATUS.FEEDBACK_IN_PROGRESS,
    );
  }
...
async function loadStrategicDiagnosticsFromApi() {
  ...
  diagnosticsApiError.value = null;
  try {
    const data = await gmKpiService.getDiagnosticsHierarchy(expected.year);
    if (!isCurrentCycleContext(expected)) return;
    ...
    diagnosticsApiError.value =
      e instanceof Error ? e.message : "Could not load strategic KPIs";
    diagnosticsApiRows.value = [];
  } finally {
    if (isCurrentCycleContext(expected)) {
...
async function loadApprovedKpiQueueFromApi() {
  ...
  try {
    const items = await gmKpiService.getApprovedKpiQueue(expected.cycleId);
    if (!isCurrentCycleContext(expected)) return;
    approvedKpiQueueApiRows.value =
      mapGmApprovedKpiQueueItemsToHierarchyRows(items);
  } catch (e: unknown) {
    if (!isCurrentCycleContext(expected)) return;
    approvedKpiQueueApiRows.value = [];
    showGmToast(
...
const diagnosticsHierarchyRows = computed(() => {
  ...
  const extra = (extraHierarchyKpisByCycle.value[id] ?? []).filter(
    (r) => !removed.has(r.id),
  );
...
function memberRowAwaitingGmFeedback(m: GmHierarchyMember): boolean {
  ...
  return (
    Number(m.assignmentStatusCode) === 407 &&
    String(m.feedbackNote ?? "").trim().length > 0
  );
}

function pmRowAwaitingGmFeedback(pm: GmHierarchyPm): boolean {
  if (pm.feedbackAwaitingGm === true) return true;
  if (pm.feedbackAwaitingGm === false) return false;
  return (
    Number(pm.assignmentStatusCode) === 407 &&
    String(pm.feedbackNote ?? "").trim().length > 0
  );
}

function hasPendingFeedbackInKpiRow(kpi: GmHierarchyKpi): boolean {
  for (const pm of kpi.pmOwners ?? []) {
    if (pmRowAwaitingGmFeedback(pm)) return true;
...
function showGmToast(
  msg: string,
  durationMs = 4000,
  variant: GmNotificationVariant = "success",
) {
...
function gmNotifyShellClass(variant: GmNotificationVariant): string {
  ...
  if (variant === "error")
    return `${base} border-rose-200 bg-rose-50/95 text-rose-950`;
  if (variant === "info")
...
async function onStrategicKpiSaved(
  ...
  const serverEdits = edits.filter((p) =>
    String(p.editingKpiId).trim().startsWith("diag-kpi-"),
  );
  ...
  for (const p of serverEdits) {
    const tid = String(p.editingKpiId)
      .trim()
      .replace(/^diag-kpi-/, "")
      .trim();
    if (!tid) {
    ...
    try {
      ...
      if (!res.success) {
        editErrors.push(
          String(res.message ?? `«${String(p.kpiName ?? "KPI")}»`),
        );
      }
    } catch (e: unknown) {
  ...
  if (creates.length === 1) {
    ...
    const years = [
      ...new Set(
        creates.map((p) =>
          String(
            cycleYearFromCycleId(String(p.cycleId ?? selectedCycleId.value)),
          ),
        ),
      ),
    ];
...
async function confirmDeleteKpi() {
  ...
  if (kid.startsWith("kpi-created-")) {
    ...
    extraHierarchyKpisByCycle.value = next;
    const plain =
      String(kpi.investigateKpiName ?? kpi.name ?? "").trim() || name;
    const d0 = departments.value[0];
    if (d0) {
      const kpis = [...d0.kpis];
    ...
    const idx =
      templateName && d0
        ? d0.kpis.findIndex((k) => k.name.trim() === templateName)
        : -1;
    if (idx >= 0) {
      for (const dept of departments.value) {
...
async function onRejectAllGmApprovedQueue(payload: {
  kpis: GmHierarchyKpi[];
  reason: string;
}) {
...
async function onRejectInactiveKpi(payload: {
  kpi: GmHierarchyKpi;
  reason: string;
}) {
  ...
  <div class="flex h-screen overflow-hidden">
    ...
    >
      ...
      <div class="h-16 flex items-center px-6 border-b border-slate-200">
        <div
          class="bg-gradient-to-br from-blue-600 to-indigo-600 text-white p-1.5 rounded-lg shadow-md mr-3"
        >
      ...
      <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        <p
          class="px-2 pt-5 text-[11px] font-bold text-slate-400 uppercase tracking-wider"
        >
          Reports
        </p>
        <RouterLink
    ...
    <main class="flex h-screen min-h-0 min-w-0 flex-1 flex-col">
      ...
      >
        ...
        >
          <div class="min-w-0">
            <p
              class="text-xs font-bold uppercase tracking-widest text-blue-600"
            >
          ...
          <div class="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <button
              v-if="
                !isGmEvaluationRoute && !isGmSettingsRoute && !gmCycleReadonly
              "
              type="button"
        ...
        <div
          class="relative flex min-h-0 flex-1 flex-col overflow-y-auto bg-slate-50/50"
        >
          ...
          <div v-else-if="!selectedDept" class="space-y-4 p-3 sm:p-4 lg:p-6">
            <GmProcessTimeline
              ...
              "
              :setting-issues="
                processTimelineData?.setting ?? activeSnapshot.settingIssues
              "
              :year-end-issues="
                processTimelineData?.yearEnd ?? activeSnapshot.yearEndIssues
              "
            ...
            <template v-if="isGmDashboardRoute">
              ...
              >
                ...
                >
                  <nav class="flex flex-wrap items-end gap-1 sm:gap-2">
                    ...
                    >
                      ...
                      >
                        <i
                          class="fas fa-table-list shrink-0 text-[11px] opacity-70"
                          aria-hidden="true"
                        />
                    ...
                    >
                      <span
                        class="flex items-center gap-2 sm:whitespace-nowrap"
                      >
                        <i
                          class="fas fa-user-tie shrink-0 text-[11px] opacity-70"
                          aria-hidden="true"
                        />
                        Evaluation
                        <span
                    ...
                    >
                      ...
                      >
                        <i
                          class="fas fa-clipboard-check shrink-0 text-[11px] opacity-70"
                          aria-hidden="true"
                        />
                        Approved KPI
                        <span
                    ...
                    >
                      ...
                      >
                        <i
                          class="fas fa-bullseye shrink-0 text-[11px] opacity-70"
                          aria-hidden="true"
                        />
                        Personal KPI
                      </span>
                ...
                <div class="min-h-0">
                  <div
                    v-show="dashboardWorkspaceTab === 'diagnostics'"
                  ...
                  <div
                    v-show="dashboardWorkspaceTab === 'pm-eval'"
                    class="p-3 sm:p-4 lg:p-5"
                  >
                  ...
                  <div
                    v-show="dashboardWorkspaceTab === 'approved-kpi'"
                    class="p-3 sm:p-4 lg:p-5"
                  >
                    <p
                  ...
                  <div
                    v-show="dashboardWorkspaceTab === 'personal'"
                    class="p-3 sm:p-4 lg:p-5"
                  >
          ...
          <GmCreateStrategicKpiModal
            ...
            :feedback-assignment-id="
              pendingGmFeedbackEdit?.assignmentId ?? null
            "
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <p class="min-w-0 flex-1 text-xs font-semibold leading-snug">
                    {{ n.message }}
                  </p>
                  <button
                    type="button"
