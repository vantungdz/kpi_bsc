...
// Khai báo kiểu dữ liệu (Bạn có thể import từ file type nếu có)
const props = defineProps<{
    rows: any[]; // Thay myKpiRows
    totals: {
        totalWeight: number;
...
// Dùng computed có get/set để v-model với textarea mà không mutate trực tiếp props
const localEmployeeComment = computed({
    get: () => props.employeeComment,
    set: (val) => emit("update:employeeComment", val),
});

// Các hàm Helper tách từ Dashboard sang
        ...
        <table class="w-full text-left border-collapse text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        <td class="p-4">
                            ...
                            <p v-if="row.certificateOutcomeNote"
                                ...
                                Thực tế (khác sheet): {{ row.certificateOutcomeNote }}
                            </p>
                        </td>
                        <td class="p-4">
                ...
                <tr class="border-t-2 border-slate-200 bg-slate-50/90">
                    <td colspan="3" class="p-4 text-right text-xs font-bold uppercase tracking-wide text-slate-700">
                ...
                <tr class="border-t border-violet-100 bg-violet-50/95">
                    ...
                    <td class="p-4 text-center text-slate-400">—</td>
                    <td class="p-4 text-center text-slate-400">—</td>
                    <td class="p-4 text-center text-sm font-bold text-violet-900">
                        {{ totals.averageScore.toFixed(2) }}
                    </td>
                    <td class="p-4 text-center text-sm font-bold text-violet-900">
                        {{ totals.averagePmScore != null ? totals.averagePmScore.toFixed(2) : "—" }}
                    </td>
                    <td class="p-4 text-center text-slate-400">—</td>
                </tr>
            </tbody>
